...
describe('the happy road', () => {
  ...
  it('writes the RIGGED bytes to OPFS, not the generated ones', async () => {
    ...
    // The generated bytes are obtained by ASKING for them here, not by spying on
    // the road — because the road deliberately no longer fetches them (#833).
...
describe('the refusals', () => {
  ...
  it('🔑 never fetches the intermediate mesh — rigging happens against a task id (#833)', async () => {
    // The defect: this road bound only `taskId` from the wide result, so the
    // service's mesh was downloaded and discarded. Measured at 7,465,804 bytes
    // on a real generation — and that discarded download is what made rigged
    // generation unreachable from a browser at all (#832).
    //
    // Asserted on the CALL, not the return value: narrowing what the caller
    // binds looks identical from the outside while still paying the transfer,
