...
describe('buildDagExportPayload (#428)', () => {
  ...
  it('stamps v11 after the clip-loop tri-state, and the stamp needs no migration on re-import (#930)', () => {
    // Pins the v10→v11 bump landmark: an accidental revert of PROJECT_FORMAT_VERSION
    ...
    const payload = buildDagExportPayload({ id: 'p1', name: 'Proj' }, emptyDagState(), 0);
    expect(payload.formatVersion).toBe(11);
    ...
    expect((reimported as { formatVersion: number }).formatVersion).toBe(11);
  });
