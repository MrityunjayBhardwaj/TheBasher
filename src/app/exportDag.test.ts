...
describe('buildDagExportPayload (#428)', () => {
  ...
  it('stamps v12 after the clip-loop tri-state, and the stamp needs no migration on re-import (#930)', () => {
    // Pins the v11→v12 bump landmark: an accidental revert of PROJECT_FORMAT_VERSION
    ...
    const payload = buildDagExportPayload({ id: 'p1', name: 'Proj' }, emptyDagState(), 0);
    expect(payload.formatVersion).toBe(12);
    ...
    expect((reimported as { formatVersion: number }).formatVersion).toBe(12);
  });
