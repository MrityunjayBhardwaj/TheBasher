...
describe('buildDagExportPayload (#428)', () => {
  ...
  it('stamps v11 after the glTF object/data split, and needs no migration on re-import (#389)', () => {
    ...
    expect(PROJECT_FORMAT_VERSION).toBe(PROJECT_FORMAT_VERSION);
    const payload = buildDagExportPayload({ id: 'p1', name: 'Proj' }, emptyDagState(), 0);
    expect(payload.formatVersion).toBe(PROJECT_FORMAT_VERSION);
    ...
    expect((reimported as { formatVersion: number }).formatVersion).toBe(PROJECT_FORMAT_VERSION);
  });
