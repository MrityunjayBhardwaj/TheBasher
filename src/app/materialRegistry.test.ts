...
const BASE: PrimitiveMaterialSpec = {
  ...
  thickness: 0.5,
  wireframe: false,
  // #532 — the two render-mode flags the native build can honour, both authored AWAY
  // from three's own default (`side` 0, `alphaTest` 0). A spec value that equals the
  // default would satisfy "the build applies it" on a build that ignores it, which is
