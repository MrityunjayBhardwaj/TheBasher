...
const formatMigrations: Record<number, FormatMigration> = {
  ...
  // v12 → v13 (#389 Stage C · C6): split each fused GltfChild into Object + GltfData.
  // Its OWN format version for the same reason as every split above: a project saved at
  // an earlier version carrying a fused imported child would never re-run an earlier pass,
...
// ── v12 → v13: the imported glTF child splits (#389 Stage C · C6) ──────────────
//
...
export function migrateFusedGltfChildToSplit(raw: unknown): unknown {
  ...
  const nodes = proj.state?.nodes;
  if (!nodes) return { ...proj, formatVersion: 13 };
