...
**FOLLOW-UP (UX #5 reopened end-to-end, 2026-06-14):** the shipped fix is only the toolbar-menu OPEN-POSITION slice. The user reopened #5 for a full menu pass — position AND behaviour across EVERY surface. Still-open menu defects of the SAME family (a placement constant that ignores the available space): (a) **submenu right-edge overflow** — both `MenuBar.tsx` `Submenu` and `AddMenu.tsx` open the child panel `absolute left-full top-0` with NO edge-flip, so a submenu off a right-side top-level menu (View) or a center-right Add menu runs off the viewport; the fix is the same shape as H91 (measure available space, flip to `right-full` when the right edge would be crossed). (b) **behaviour, not position:** standard menubars HOVER-SWITCH between open top-level menus and support arrow-key nav — `MenuBar.tsx` is click-only. A shared edge-aware menu primitive (V34) would fix all surfaces at once. See UX-BACKLOG #5.
