...
**FOLLOW-UP (still OPEN, wider pre-existing bug, NOT drill):** the per-child TRS/material/suppress paths in `GltfAssetR` still address children by name via `nameToObject.get(key)`, so editing an override on a name-mismatched GltfChild silently no-ops on the rendered clone. The `userData.basherGltfChildId` stamp now present on every clone object is the hook to fix it (resolve override children by stamped id too) — tracked separately; the drill fix stands independently.
