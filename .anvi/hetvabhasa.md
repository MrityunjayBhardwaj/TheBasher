...
**Real fix:** raise the budget on CI only — `timeout: process.env.CI ? 60_000 : 30_000` in `playwright.config.ts`. One place covers the whole span (every heavy e2e, present + future); keeps local fast-fail at 30s to catch genuine hangs while iterating. This is calibration, not race-papering: race-papering masks nondeterminism with time; here there is no nondeterminism to mask (proven deterministic), only a budget mis-sized vs the real per-test work on the actual runner.
