...
**Status: OPEN — root cause diagnosed on the cicada/CountryLands glb (2026-06-13, `fix/ux-friction-4`, UX #7). Drill-in (`eefbec2`) works on the `multifile/flat` fixture but NOT on a real dense model. Fix deferred to a dedicated session (stamp `userData` ids at render, drill by id not name).**
