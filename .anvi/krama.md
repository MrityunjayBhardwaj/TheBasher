...
**REF:** P6 W9 commits C3 `28d6a3b` (static layer + offscreen + dims-independent attr effect) + C4 `28350ab` (rAF strip-redraw + idle/stale-closure guards + sentinel reset); `src/timeline/TimelineCanvas.tsx`; `src/timeline/timelineCanvasGeometry.ts` (the pure geometry the shell is thin over — `playheadStripRect`, `PLAYHEAD_STRIP_HALF_WIDTH_PX`); `tests/e2e/p6-w9-timeline-canvas.spec.ts` (scrub/cull/ref-sync/R3F-no-remount) + `tests/e2e/p6-w9-perf.spec.ts` (the 240-frame ≥60fps Lokayata gate, observed p95≈9ms). Depends on [[V20]] (currentFrameRef single-writer) and the V8 mount-once rider (the new 2D canvas must not perturb the R3F Canvas — asserted, not assumed). Sister: K1 step 6 (R3F Canvas mounts once — same "don't remount the heavy canvas" family, different canvas).
