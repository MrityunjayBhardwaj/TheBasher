...
## K19 — The ComfyUI keyframe→schedule COMPILE lifecycle (ingest → bind → bake → classify → insert-schedule → rewire → submit → collect → stitch)

The ordered pipeline that turns a Basher-authored ComfyUIWorkflow layer into a rendered clip. ONE baker feeds two compile TARGETS (preview = per-frame substitution; compiled = batched schedule) so preview==compiled==dopesheet (the [[V81]] thesis). Steps:

1. **Ingest** — `importComfyGraph(apiJson, meta)` (`comfyGraph.ts`) walks every node's LITERAL inputs → a param manifest with `valueKind` + `scheduleHint` (SCHEDULABLE vs STRUCTURAL, §7.4). Pure, GPU-free, snapshot-tested (the IP). Links (`[id,idx]`) are skipped — wired, not authored.
2. **Bind** — a free-floating `KeyframeChannel*` ([[V57]]) targets the node at `comfy:<nodeId>.<inputName>` (NO new keyframe machinery). Authored in the Controls panel (Slice D) by valueKind dispatch ([[H104]]).
3. **Bake** — sample each schedulable param via the render-identical `resolveEvaluatedParam` ([[H40]]): preview bakes at ONE frame (`resolveComfyParamsAtFrame`), compiled bakes the ARRAY over [frameStart,frameEnd] (`bakeComfyBatchedTracks`). SAME sampler → no drift.
4. **Classify + 5. Insert-schedule + 6. Rewire** — `compileBatchedWorkflow`: float/int VARYING tracks → insert a `BasherValueSchedule` carrying the baked array + rewire `inputs[name]=[schedId,0]` + set `batch_size=N`; CONSTANT tracks → substitute the literal, NO node ([[H127]]); STRUCTURAL/string/image → DEMOTED to a `demotions` list (kept literal, never silent §7.4). Preview's analogue is `compilePreviewFrame` (substitute, no schedule node).
7. **Submit** — `cap.submit` (preview, single-frame) or `cap.submitBatch` (compiled, N frames); the batched path checks `comfyHasNodeTypes` first (the bridge node must be installed — §16 Q-E) and uses `batchTimeoutMs` not the 30s single-frame budget ([[H126]]).
8. **Collect** — Http `submitBatch` gathers EVERY output image across ALL nodes (a batched SaveImage emits N); the stub returns N deterministic frames.
9. **Stitch** — frames → `createMp4Sink` (the SAME 3D-render encoder) → OPFS → a project video MediaClip (`compileComfyBatch`), reusing the `saveRenderPassesToProject` on-ramp.

**Common violations:** introducing a SECOND keyframe sampler for workflow params instead of reusing `resolveEvaluatedParam` → preview/compiled/dopesheet drift; emitting a schedule node for a CONSTANT param → needless bridge-node dependency + a server-without-the-extension rejects the un-animated render ([[H127]]); reusing the single-frame timeout for the batch → abort-on-success ([[H126]]); collecting only the first output image in `submitBatch` → a batch returns one frame; routing a compile/submit failure to the view3d-covered banner instead of the app-root toast ([[H122]]); silently dropping a STRUCTURAL/unsupported param from the compiled path instead of LOGGING the demotion (§7.4). REF: `src/core/comfy/comfyGraph.ts` (ingest + both compiles), `src/app/video/compileComfyBatch.ts` (bake-range + the action), `src/core/comfy/{Http,Stub}ComfyUICapability.ts` (submit/submitBatch), `src/app/video/compositeDecode.ts` (preview decode), `custom_nodes/BasherSchedule/` (the GPL/MIT bridge, separate repo). Cross-ref [[V81]] (the epic), [[V57]] (the channel road), [[K16]] (the 3D render lifecycle sibling), [[H40]]/[[H122]]/[[H126]]/[[H127]], dharana B24.
