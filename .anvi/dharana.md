...
### Boundary B14: Evaluated-mesh boundary (`resolveEvaluatedMesh` — one producer, many consumers) — PLANNED (v0.6 #1 / #150)
...
**HOW (the focus + observation targets) — to be GROUNDED with file:line REFs in Wave 5 of the phase:**

- `resolveEvaluatedMesh` is the SOLE producer; the renderer (`SceneFromDAG.tsx` BoxMeshR/SphereMeshR/GltfAssetR) and EVERY read-side surface (gizmo, NPanel) must apply each band identically at the SAME `ctx.time` — the "one band, two callers" BLOCK-1 rule already enforced by [[resolveGltfChildTransform]] ([[B1.1]] template).
- **Silent-failure modes:** (a) a band threaded into the resolver but not the renderer → gizmo shows X, viewport renders Y (H40); (b) a post-release EvaluatedMesh field read without a `?? default` at a consumer → hydrate-seam crash on old projects (V10/H14 two-layer guard); (c) a schema migration (primitive gains `scale`) that changes an existing project's render → must default to identity, proven byte-identical.
- **Observation target:** the boundary-pair e2e reads the REAL rendered three.js object value (scene-walk seam, mirror `__basher_gltf_meshes`) AND the resolver value, asserts equality at ≥2 states — never infer from node params alone ([[H58]]).
- **REF (grounded during execution):** `src/app/resolveEvaluatedMesh.ts` (new), `src/viewport/SceneFromDAG.tsx` (BoxMeshR/SphereMeshR scale + read seam), `src/app/Gizmo.tsx`, `src/app/NPanel.tsx`/`resolveTransformParam.ts`, the geometry registry, `tests/e2e/p150-evaluated-mesh.spec.ts`, `.planning/phases/v06.1-unified-mesh-model/{CONTEXT,RESEARCH,PLAN}.md`. Mirrors [[B1.1]] · [[H40]] [[H36]] [[V20]] [[V10]]. Issues #150 (this), #149/#151/#2/#3 (riders).
