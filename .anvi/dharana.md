...
**Open follow-ups now filed at this boundary (post-PR #92):** #88 (skinned glTF → Skeleton + AnimationClip chain — the bone-indexed sibling of the TransformClip TRS path); #89 (quantised animation accessors / KHR_mesh_quantization); #90 (data-URI buffers / JSON-only `.gltf` external buffer resolution; #82's load-layer cousin); #91 (glTF child gizmo/NPanel addressing — `resolveEvaluatedTransform` consumer evolution for glTF scene children; H40-family at this boundary). The TransformClip / ClipSelect node pair (PR #92) is the canonical shape — any future clip importer (FBX node-indexed clips, Alembic, USD) should mirror its (importer-chain → ClipSelect → consumer) topology, not invent a parallel one.
