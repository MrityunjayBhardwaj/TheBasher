...
**Silent-failure modes:** a scene-authored singleton mounted as the editor's instrument has no selectable body (you're inside it); two complementary `makeDefault` cameras race over `state.camera` on toggle; look-through that mounts a 2nd render camera fights drei's oldCam-restore; persisting the look-through pose as the editor view (it's the DAG camera's, not the user's free view) — gated by the `!lookThroughCamera` guard on save.
**Observation targets:** real-canvas e2e — (1) boot view camera == active camera pose (byte-identical first paint), (2) clicking a camera frustum selects the camera NODE (falsify: disable CameraHelpers → click hits the cube), (3) Numpad 0 → view adopts the DAG pose + CAMERA VIEW badge, 0 again exits, (4) orbit → reload restores the saved pose, distinct from default. The DEV `__basher_view_camera` + `__basher_project_ndc` seams observe the real view camera. Any future scene-entity-as-instrument MUST land a sibling "can I see/select it AS data?" gate.
