...
**STATUS (2026-06-07, branch `feat/v06.2-material-editing`, W0-W5 SHIPPED + falsified; W6 per-submesh glTF NOT YET BUILT):** R1 (migration look-shift) OBSERVED + gated — `migrations.test.ts` proves migrated box = 0.5 / fresh = 0.3, the two-defaults split cannot silently converge (kept as the boundary's anticipation, not promoted; it was a designed default-split, not a bug). CAVEAT-1 (guard narrowing) OBSERVED + handled — `isMaterialSpec` rewritten dual-accept (`base.color` OR legacy `color`) BEFORE it bit; gated by a unit assert + the migration render path. **Two NEW patterns promoted this execution:** [[H73]] (`git checkout/restore <file>` to undo a falsification NUKES uncommitted wave work → commit-before-falsify) and [[H74]] (a paramPath rename's span includes the AGENT layer + LLM prompt + e2e corpus + sibling renderers, far beyond the V10/H14 schema-consumer list — the plan's "5-consumer span" for `material.color→material.base.color` under-counted; grep all of src+tests+doc-strings before renaming a paramPath). **W6 OPEN at this boundary:** the second write path (glTF child → per-`(child,slotIndex)` MaterialOverride, [[V28]]/[[H59]]) + the per-submesh slot selector UI + backward-compat with whole-child #99/#124 — the largest-risk piece, fixture `public/assets/two-material-quad.gltf` (6.0) committed, `GltfAssetR` collapse grounded (one override → every slot at `SceneFromDAG.tsx` traverse, `m.material = Array.isArray(src) ? src.map(make) : make(src)`).
