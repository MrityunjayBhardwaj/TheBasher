...
### Boundary B22: Operator substrate (constraints/modifiers as typed DAG chains) + the world-transform-purity gate — PLANNED (epic #201, NOT STARTED)
...
**Observation targets:** slice #202 — a boundary-pair test: pure `resolveWorldTransform` == the rendered world matrix (`__basher_mesh_world_position` seam) for a nested Transform/Group hierarchy at ≥2 playhead times (falsify: revert the pure lift → values diverge from render). Slice #204 — camera look-through/still/animation render all match through the migrated Track-To (reuse p190/p194 gates); a node-ref-targeted constraint follows a moving target, viewport == render; a cycle (A→B→A) is rejected by the evaluator. Status: **PLANNED, NOT STARTED** — re-validate this boundary when slice #202 begins.
