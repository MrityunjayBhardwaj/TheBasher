...
**NEXT CONCRETE EXERCISE (v0.6 #4 Director-UX chrome, branch `feat/v06.4-director-ux`):** the chrome/advanced-UI seam gets its first real test. (1) The **D-05 mode removal** (delete Simple/Director/Pro — `ModeSwitcher`/`ModeBadge`/`chromeStore`/`Layout`/`TopToolbar`) must NOT replace modes with a bolted-on "advanced" surface — everything stays the SAME projected graph, revealed by progressive disclosure (leak-check #3). (2) The **toolbar consolidation** (4 bands + left rail + bottom bar → one floating toolbar, target `docs/SPLINE-UI-REFERENCE.md`) must keep tools as ONE surface, not duplicate the select/move/rotate/scale controls (the current rail⊕bottom-bar duplication is the F1 friction). Every chrome wave ALSO runs the §B11 SECTION-INVENTORY re-validation ([[H25]]/[[H27]]), the [[H75]] conditional-render-strands-a-value check, and W8 a11y. Decided-after-audit (D-02): the `/anvi:ui-review` 6-pillar pass settles restyle-vs-consolidate for W1.
