...
**NEXT CONCRETE EXERCISE (Spline-exact chrome REDESIGN — branch `feat/v06.4-director-ux`, 2026-06-09):** v0.6 #4 Director-UX is feature-complete (W1-W5; W5 `cc73533`). The user then said the editor "doesn't look anything like Spline 3D — I want it exactly like this end to end" (showed the Spline editor: dark, ambient-glow, rich left outliner + right inspector, clean sans). Observed the gap live (screenshot): shipped chrome was LIGHT + monospace + dual cramped panels = a dev tool. **Scope LOCKED with user via AskUserQuestion: (1) DARK Spline-true (reverses the W3 light call — see [[V36]] bidirectional inversion); (2) KEEP the agent first-class, Spline-styled (NOT literal-Spline-no-agent — §196/D-03/[[V34]] thesis holds); (3) editor SHELL + home only (NOT the hero inflated-glossy material).** Plan = 6 waves: **A foundation (DONE `38c925c`) → B left scene outliner → C right inspector (+ re-home agent Spline-style) → D top toolbar + bottom orbit-gizmo/ortho-persp → E home launcher → F polish + a11y re-baseline + screenshot-diff.** Wave A flipped tokens light→dark (accent → Spline blue `#5c9dff`), monospace→system-sans (repointed `mono` token; 17 `font-mono` classes flip globally — legacy name, later cleanup), index.css ambient corner-glow, dark viewport `#1a1b20`; re-ground the [[V36]]/[[H27]] gates in-wave (#57 worst-case DARK→BRIGHT, F7 literal, p57 re-grounded → surfaced [[H83]]). **NEXT WRITE = Wave B (left outliner): always-on Spline panel — header + search + scenes + icon tree + Library/Import/Help footer; restyle SceneTree rows to Spline (icon + name + subtle blue-ring selected).** None merged; #4 stacks on #3; main still `a5fd4df`. NEW DEV seam `__basher_three` (threeRef) from W5. Per-wave gate UNCHANGED (tsc -b/eslint0/format/vitest/falsified-e2e/W8+contrast on chrome DOM/section-inventory H25/H27); the [[V36]] inversion re-grounding is now a ROUTINE step on any token-polarity change.
