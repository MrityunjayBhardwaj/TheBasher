...
**NEXT CONCRETE EXERCISE (v0.6 #4 Director-UX chrome — branch `feat/v06.4-director-ux`, issue #185, PLAN.md 5 waves W2→W1→W3→W4→W5; W2 `b5af221`, W1 `bda0cac`, W3 `29a3afe`, W4 `ff374ec` — only W5 NOT yet, 2026-06-09):** the chrome/advanced-UI seam's first real test. Premise corrected at audit ([[H78]]): the modes to remove are the OPERATIONAL enum `edit/run/animate/director` (`modeStore.ts:23`), NOT the density Simple/Director/Pro (those left under D-UX-5) — D-05 dissolves the enum FULLY (run→Play, animate→timeline-drawer reveal, director→presentMode toggle). **NEXT WRITE = W5 (first-run polish: visual ADD palette reusing AddMenu, NPanel empty-state hints, click-to-select regression e2e) — the lightest wave, independent of W3/W4. None merged; #4 stacks on #3; main still `a5fd4df`.**
