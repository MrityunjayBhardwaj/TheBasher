...
**NEXT CONCRETE EXERCISE (v0.6 #4 Director-UX chrome — PLANNED 2026-06-08, branch `feat/v06.4-director-ux`, issue #185, PLAN.md 5 waves W2→W1→W3→W4→W5, NOT yet executed):** the chrome/advanced-UI seam gets its first real test. Premise corrected at audit ([[H78]]): the modes to remove are the OPERATIONAL enum `edit/run/animate/director` (`modeStore.ts:23`), NOT the density Simple/Director/Pro (those left under D-UX-5) — D-05 dissolves the enum FULLY (run→Play, animate→timeline-drawer reveal, director→presentMode toggle).

**Execution observation targets (run at the Write to each seam — these are the leak-check + the per-wave traps the plan PINS):**
- **W2 mode removal** must NOT replace modes with a bolted-on "advanced" surface — every revealed surface stays the SAME projected graph via progressive disclosure (leak-check #3). The deletion regression **grep gate** (`grepGates.test.ts`, V19 home) must match `modeStore`/`useModeStore` by import-path+symbol and EXCLUDE two homonym vocabularies (`useAgentSessionStore.setMode` = agent autonomy KEEP; `gizmoStore.setMode` = tool dispatch KEEP) AND scrub the `modeStore` COMMENT token at `FloatingViewportToolbar.tsx:30` (the gate greps comments) — else it never goes green. The `presentMode`/`routeStore` state added is EPHEMERAL UI-projection (non-persisted, like `chromeStore`) — V34-clean, NOT authoritative scene/IR state (confirm).
