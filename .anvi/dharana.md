...
**Observation targets:** for any ComfyUI-touching change, ask "did I observe THEIR side (the `/system_stats` / `/history` record) or only infer from my own request?" and "does an unreachable/misconfigured server FAIL LOUDLY (a surfaced reason) rather than silently stub?". Drive the boundary by snapshotting the compiled workflow JSON (deterministic, no GPU) AND, when a real server is up, the `/prompt`→`/view` round-trip (the green-cube method). Inc 1+2 SHIPPED; NEXT = Inc 3 (keyframe-any-param preview, provable vs the stub first) → Inc 4 (`submitBatch` + the bridge node + MP4).
