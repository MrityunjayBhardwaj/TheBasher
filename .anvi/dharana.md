...
**NEXT CONCRETE EXERCISE (v0.6 #4 Director-UX chrome — branch `feat/v06.4-director-ux`, issue #185, PLAN.md 5 waves W2→W1→W3→W4→W5; W2 committed `b5af221` 2026-06-08, W1 committed `bda0cac` 2026-06-09, W3/W4/W5 NOT yet):** the chrome/advanced-UI seam's first real test. Premise corrected at audit ([[H78]]): the modes to remove are the OPERATIONAL enum `edit/run/animate/director` (`modeStore.ts:23`), NOT the density Simple/Director/Pro (those left under D-UX-5) — D-05 dissolves the enum FULLY (run→Play, animate→timeline-drawer reveal, director→presentMode toggle). **NEXT WRITE = W3 (calm LIGHT palette, D-07) → W4 (Spline HOME, D-08/D-09) → W5 (first-run polish).**
