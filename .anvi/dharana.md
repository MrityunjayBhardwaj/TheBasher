...
### Boundary B-gizmo: the selection/manip per-frame cost (TransformControls + resolveEvaluatedTransform + inspector) — NEXT TARGET, NOT YET PROFILED
ORIGIN: 2026-06-12 cicada bottleneck hunt ([[H48]] 6th occ). The "manipulate a node → 120→2fps" the user reported localizes ENTIRELY here, not in the renderer: a mounted gizmo costs ~117ms/frame (box-edit 38ms → box+gizmo 155ms), and editing a SELECTED cicada child costs ~414ms more (→570ms/frame, 2fps) — react is only ~38ms of it, so the bulk is non-React per-frame work.
WHY: without this boundary tracked, the next session repeats the 3-session mistake of optimizing GltfAssetR (which is <2ms/frame, proven by microbench) instead of the gizmo path where the time actually is.
HOW: profile, BREAKDOWN-FIRST, the per-frame cost of (a) drei `<TransformControls>` mounted on a 700-node/466K-tri scene (raycast? re-render? matrix?), (b) `resolveEvaluatedTransform`/`resolveTransformParam` the manip proxy + inspector read every frame (it calls `bakedChannelSamplersForAsset(state.nodes)` = a full O(nodes) walk per call), (c) the inspector NPanel re-render on each edit. Files: `src/app/Gizmo.tsx`, `src/app/resolveEvaluatedTransform.ts`, `src/app/resolveTransformParam.ts`, `src/app/NPanel.tsx`. Seams: `__basher_gizmo`, `__basher_selection`, `__basher_perf` (frame/react). REF: [[H48]] 6th occ, [[B13]], [[V42]].
