...
**HOW:** one pure resolver (`resolveEvaluatedTransform`) mirrors the renderer's scene-child index-correspondence + unwraps the AnimationLayer patched clone (Chesterton — the SceneFromDAG mechanism already exists; do not invent a parallel walk). Every surface needing "where it actually renders" consumes that one helper. Consumers (2): (1) **Gizmo** (`src/app/Gizmo.tsx`) — direct consumer; the manip proxy SEEDS from the resolver each playhead change (the seeding `useEffect`). The old D-01 "layer-select synthesize" branch (`evalForSelection` — fabricate a manip when the selected node had no `position` param but the resolver resolved it) was REMOVED in #194: it existed only for the positionless AnimationLayer wrapper, and post-#199 every positionless-param node (Group/GltfAsset) also evaluates to a positionless value, so the resolver returns null → the synth could never fire. The gizmo's `manip` now comes straight from `getManipulable(node)`. (2) **NPanel Inspector** (`src/app/NPanel.tsx`) — via the transform-param adapter `src/app/resolveTransformParam.ts` (per-param view over the resolver; the read-only-while-playing gate is `playing && resolved !== null`). The reusable diagnostic question: "which side of the producer/consumer boundary did I observe — the evaluator, or the surface?"
...
**#199 re-derivation (2026-06-18, Slice C):** the HOW above ("unwraps the AnimationLayer patched clone") is superseded — `resolveEvaluatedTransform` now **overlays the free-floating direct channel** ([[V57]] / `overlayChannels`) onto the matched scene child; there is no wrapper to unwrap (the matched scene child IS the producing node). The boundary, the two callers (Gizmo + NPanel), and the WHY are UNCHANGED — the resolver is still the ONE "where it actually renders" helper both consume; only the internal mechanism changed (unwrap clone → overlay channel). The named "evaluate the node" trap is now SHARPER: no node's `evaluate()` carries the animated value at all (the channel is free-floating, an input to nothing), so reading `evaluate(node)` gives the raw value — see new [[H107]]. No shift to the boundary; mechanism re-pointed.
2026-06-03 (#141 / PR #142): the gizmo caller was ASYMMETRIC — it invoked the chokepoint's GUARD (`routeAnimatedGrab`) but not its COMMITTER (`autoKeyCommit`), so drag-to-record created no first keyframe while the inspector did ([[H63]]). Fixed by adding `autoKeyCommit` after the raw `setParam` in all three gizmo onObjectChange branches, mutually exclusive with the animated early-return (no H36 double-write). The chokepoint is now symmetric across both callers; `tests/e2e/gizmo-autokey-record.spec.ts` is the regression gate.
...
### Boundary B1.2: Evaluation-wrapper ↔ SELECTION (the wrapper must never BE the selected entity)

**ORIGIN:** 2026-06-04 (#160 + #162). User friction, TWO surfaces in one session: (a) "the gizmo moves, not the object" after keyframe→scrub→drag (#160), and (b) "transform and material properties are not visible all the time… something deeply wrong in this context" (#162). Root cause for BOTH: keyframing wraps the object in an `AnimationLayer` that becomes the SCENE CHILD; a viewport CLICK then selects the LAYER (`SceneFromDAG.tsx` `pickId = childRefs[i].node`). The gizmo's grab check (`paramAnimationState(layerId)`) saw the layer as un-animated → no transient (#160); the inspector rendered the layer's `inspectorSections = ['animate']` → no transform/material (#162). This is B1.1's sibling: B1.1 = the wrapper's evaluated VALUE leaks (stale surface); B1.2 = the wrapper's IDENTITY leaks (you select the wrapper, not the object).
**WHY:** Without this entry, the NEXT evaluation-wrapper (a future modifier stack, constraint, deformer — anything that wraps a node and becomes a selectable scene child) re-introduces a selectable wrapper that hijacks selection, and EVERY surface reading `selectedNodeId` (inspector, gizmo, dopesheet, UV editor, agent context) breaks one bug at a time. Removing this entry reopens the class as empirical rediscovery. GROUNDED (the deductive gate): Blender keeps animation in `object.animation_data` attached to the object — the object stays the selected/inspected entity, animation shown by color + a dedicated editor (Graph/Dope Sheet/NLA pane). Houdini keys parms ON the node — "the node remains the primary selectable entity"; even Animation LAYERS are a separate pane working ALONGSIDE selection. **Universal principle: the evaluation wrapper is an implementation detail; selection + the properties panel + the gizmo always address the OBJECT; "this is animated" is communicated by color + a dedicated animation editor, never by changing what you select.**
**HOW:** ONE shared pure `resolveEditTargetId(state, selectedId)` (`src/app/animate/resolveEditTarget.ts`) — single-hop unwrap of an `AnimationLayer` selection to its `inputs.target`, mirroring `SceneFromDAG`'s `animationTargetId` shape (no parallel walk); identity for non-layer. Applied as the ROOT fix at the SELECTION boundary: the viewport mesh `onClick` (`SceneFromDAG.tsx`) resolves layer→object so a click ALWAYS lands on the object; the layer stays reachable ONLY via the SceneTree (graph view), matching Blender's NLA / Houdini's Animation-Layer pane being separate. Plus safety-net unwraps on the GRAB + READ paths (`routeAnimatedGrab` + `autoKeyCommit` + `resolveEvaluatedTransform` overlay key). Prefer the ONE root unwrap at the selection boundary over N per-surface unwraps.
**Silent-failure modes:** a viewport click selects an evaluation-wrapper instead of the object → the inspector loses the object's sections (shows only the wrapper's), the gizmo edits/checks the wrong node (frozen object), the dopesheet/UV editor/agent see the wrapper id. The tell: "I keyframed it and now the panel is empty / the gizmo doesn't move it."
**Observation targets:** a REAL viewport click on a keyframed object → `selectedNodeId == the object` (NOT the layer) AND the inspector still has transform+material (`tests/e2e/p162-layer-select-unwrap.spec.ts`); a gizmo drag with the layer selected → rendered object moves (`tests/e2e/p160-gizmo-layer-select.spec.ts`). For any NEW wrapper node type: add a "does a click select the wrapper?" check before shipping it as a scene child.
**REF:** issues #160 + #162, hetvabhasa [[H40]] (#160 grab-side + #162 selection-side extensions), [[B1.1]] (the value-leak sibling), `src/viewport/SceneFromDAG.tsx` (onClick), `~/.anvideck/projects/basher/ref/GROUND_TRUTH_BLENDER_KEYING.md` (Blender model), Houdini parms/animlayer docs (sidefx.com). PR #161.
**#199 re-derivation (2026-06-18, Slice C, `bd8f7e2`) — the boundary is now satisfied by ELIMINATION, not by unwrapping.** The `AnimationLayer` wrapper is gone (V57): a keyframed node is its OWN scene child (a free-floating channel reaches it by `params.target`, never re-parents it), so a viewport click `pickId = childRefs[i].node` already IS the object — there is no wrapper to hijack selection. `resolveEditTargetId` (the single-hop unwrap) was DELETED and its callers (`SceneFromDAG` onClick/onDoubleClick, `autoKeyCommit`, `resolveEvaluatedTransform`) now use the selected id directly; p160/p162 were deleted (the machinery they tested is gone). **The WHY stays load-bearing** — the NEXT evaluation-wrapper that becomes a selectable scene child (a future modifier stack / deformer) reopens the exact class; this entry is the deductive check ("does a click select the wrapper?") to run before shipping any such node. Basher's current answer is the strongest: there is no wrapper, so the failure mode is structurally impossible — but the principle (selection addresses the OBJECT, animation shown by colour + a dedicated editor) is what must hold for any future wrapper. This boundary is now a GUARD against regression, not an active-fix site.

### Boundary B16: Editor view camera ↔ DAG scene cameras (the #165 instrument/data split)

**ORIGIN:** issue #165 (2026-06-05) — the DAG camera mounted as the viewport's `makeDefault` render camera, so the editor looked THROUGH the scene camera. User friction: "I see the gizmo but not the camera, I can't select it." A scene-authored entity was also the editor's instrument, so it could never be seen/selected AS data ([[H67]]).
**WHY:** Without this boundary tracked, the next scene-entity-as-editor-instrument (an audio listener at the ear, a "light you stand in", a future VR rig) repeats the conflation and ships an unselectable first-class object — diagnosed from scratch instead of matched to the H67 instrument/data-split pattern. It also re-attempts the drei `makeDefault` two-camera toggle that RACES (oldCam restore on flag-flip).
**HOW:** the editor owns ONE always-default free orbit camera (`EditorViewCamera`); DAG cameras are NOT `makeDefault` — they render as selectable wireframe frustum proxies (`CameraHelpers`, sibling of `LightHelpers`, enumerated from `state.nodes`). "Look through camera" (Numpad 0 / toolbar / menu) = the editor camera ADOPTS the active DAG camera's pose+projection (orbit gated off), never a second `makeDefault`. Editor view pose is a per-project UI projection in localStorage (`editorViewPersistence`), NOT scene data (Blender stores the user viewport with the workspace). The active camera's pose is read via the pure `activeCamera` helpers.
**REF:** issue #165, hetvabhasa [[H67]] (the pattern), drei `makeDefault` grounding `node_modules/@react-three/drei/core/PerspectiveCamera.js:52-64`, `src/viewport/EditorViewCamera.tsx`, `src/viewport/CameraHelpers.tsx`, `src/app/activeCamera.ts`, `src/app/editorViewPersistence.ts`, `src/viewport/SceneFromDAG.tsx` (camera enumeration; no makeDefault), `src/viewport/Viewport.tsx` (orbit gate + onEnd persist), `src/app/{KeyboardShortcuts,FloatingViewportToolbar,MenuBar}.tsx` + `src/viewport/ModeBadge.tsx` (look-through affordances), `tests/e2e/p165-blender-camera.spec.ts`.
**Silent-failure modes:** a scene-authored singleton mounted as the editor's instrument has no selectable body (you're inside it); two complementary `makeDefault` cameras race over `state.camera` on toggle; look-through that mounts a 2nd render camera fights drei's oldCam-restore; persisting the look-through pose as the editor view (it's the DAG camera's, not the user's free view) — gated by the `!lookThroughCamera` guard on save; **the editor-view boot guard is a lifetime-scoped boolean (`didInit`) while the Canvas mounts once → in-session project switch doesn't re-frame; only reload fixes it ([[H69]], #167) — the guard must be keyed to the project id, not the component lifetime.**
**Observation targets:** real-canvas e2e — (1) boot view camera == active camera pose (byte-identical first paint), (2) clicking a camera frustum selects the camera NODE (falsify: disable CameraHelpers → click hits the cube), (3) Numpad 0 → view adopts the DAG pose + CAMERA VIEW badge, 0 again exits, (4) orbit → reload restores the saved pose, distinct from default, (5) **orbit project A → duplicate (in-session switch) → view re-frames to the new project's active-cam pose (`p167`; falsify by reverting the project-keyed guard to `=== null`).** The DEV `__basher_view_camera` + `__basher_project_ndc` seams observe the real view camera. Any future scene-entity-as-instrument MUST land a sibling "can I see/select it AS data?" gate; any app-lifetime singleton holding per-project state MUST key its boot guard to the project id (H69). **2 error patterns now cluster here (H67 instrument/data + H69 lifetime-scoped guard) — at 3, run the fatality test on the editor-view ↔ DAG-camera boundary.**
**#12 re-validation (2026-06-15):** the camera node is now the inspector's lens-authoring surface too (Camera section: focal-length+sensor→FOV, near/far, DoF). `fov` stays the single rendered source of truth ([[V51]]) so the lens drives this boundary's view (look-through + free orbit) with NO new camera pipeline — `pose.fov` already flows through `EditorViewCamera`. NO SHIFT to the instrument/data split. New trap is at B17 (render ownership), not here.
**#186 re-validation (2026-06-16):** the boot/re-frame branch now does FULL BOUNDS-FIT on load ([[V54]]) — the editor view frames the scene's bounding sphere with near/far + orbit limits derived from the radius, via an async `useFrame` settle loop that yields to canvas input (one-time framing, NOT a center constraint — user requirement). Built ON the #167 project-keyed guard (H69's lifetime-guard pattern is the foundation, NOT re-triggered → no new clustering). NO SHIFT to the instrument/data split — the fit affects only the FREE-view pose/planes, never look-through (still adopts the DAG camera). New observation target (6): a large model boots framed with far≫1000, no clip (`p186`). KNOWN GAP **#191** — saved-view & projection-toggle branches skip the settle → keep 0.1/1000 planes → reload-clip; planes must derive from LIVE camera distance there too. Still 2 error patterns (H67/H69), no fatality escalation.
**#190 re-validation (2026-06-17):** a keyframed camera is now FOLLOWED across this boundary by ONE pure resolver `resolveActiveCameraPoseAt(state, seconds)` ([[V56]]) — look-through samples it each frame from LIVE time+DAG snapshots (never a time subscription, [[H48]]), and the still + animation render consume the SAME resolver (parity, [[V37]]/[[V51]]). NO SHIFT to the instrument/data split: the resolver overlays channels onto the SAME camera node this boundary already reads; look-through still adopts the (now time-evaluated) DAG pose. Camera channels target the camera node DIRECTLY (no AnimationLayer — the camera is wired via `scene.camera`, outside the scene-child layer machinery); first-key is the camera-specific `dispatchCameraFirstKey`, NOT the addLayer composite. New trap surfaced at the INSPECTOR boundary, not here: a node with a custom inspector control (`CameraLensControls`) bypasses the generic diamond/autoKey → its params are silently non-animatable until the diamond + autoKey are wired INTO the control ([[H104]]). New observation target (7): look-through scrub animates the view camera; the inspector fov diamond creates a free-floating channel with no layer (`p190`). Still 2 error patterns at THIS boundary (H67/H69); H104 clusters at the inspector-control boundary (B-inspector), not here. DEFERRED: animated DoF (#193), dopesheet labels for camera channels (#194 follow-up).
**#194 re-validation (2026-06-18) — the inspector READ-side is now closed, [[H104]] CLOSED.** `CameraLensControls` was the LAST custom-control inspector whose read side stayed static: it read `node.params.fov/near/far` directly, so during scrub the focal/FOV/near/far fields froze while look-through (`resolveActiveCameraPoseAt`) moved — the [[H40]] half of H104. Fixed with the #198 `resolveEvaluatedParam` template (subscribe to time, resolve the animatable scalars per-node, `effective = resolved ?? authored`, `readOnly = playing && resolved !== null`); `resolveEvaluatedParam`'s `.sample()` bottoms out in the SAME `sampleScalarKeyframes` as `resolveActiveCameraPoseAt`, so inspector == look-through (the V56/B1.1 boundary-pair, gated by `p194-camera-read-side.spec.ts`). NO SHIFT to the instrument/data split — the read side consumes the same camera node this boundary already reads; it adds no camera pipeline. Held-transient preview comes FREE (resolveEvaluatedParam layer-1 IS the transient), so that #194 sub-item is also closed; only dopesheet labels for camera channels remain a follow-up. With this, every custom-control inspector carries the full diamond/autoKey/evaluated-read/read-only spine — the H104 cluster at B-inspector is resolved, not active.
**#229 re-validation (2026-06-23) — the camera is now MANIPULABLE across this boundary (rotate + aim handle + roll), [[V77]].** A camera had `position`+`lookAt`(point) with NO rotation param, so the gizmo coerced rotate→translate and the aim point had no handle. The mapping between authored (position, lookAt, roll°) and the camera's world ORIENTATION is now ONE pure bijection (`src/app/cameraOrientation.ts`: `cameraOrientationQuat` forward + `lookAtRollFromQuat` inverse). EVERY pose→orientation consumer at THIS boundary derives orientation forward from it — look-through (`applyView`), frustum (`frustumQuaternion`), still+anim render (`buildRenderCamera`), world-seam (`cameraWorldMatrix`) — so render==look-through==frustum at any roll ([[V37]], no second orientation path to drift). The `CameraGizmo` (Gizmo dispatcher's 3rd branch) manipulates in world orientation and writes authored lookAt+roll back via the inverse ([[V68]] for cameras): rotate orbits the aim at fixed distance + banks roll, a 2nd translate AIM handle writes lookAt directly, translate moves position only (lookAt point stays put). `roll` is a V56 camera SCALAR channel (resolver overlays it like fov/near/far) + a generic TRANSFORM inspector row (keyframeable via the GENERIC diamond — NOT inside `CameraLensControls`, so it sidesteps the H104 trap closed in #194). NO SHIFT to the instrument/data split — the gizmo edits the SAME camera node this boundary reads; no new camera pipeline. NEW silent-failure mode at this boundary: a partial/legacy pose with `undefined`/NaN `roll` poisons the orientation quaternion with NaN → the camera VANISHES, no throw ([[H118]] — finite-guard roll→0 at the pure helper, defaulted at `cameraPoseFromNode`). New observation target (8): the aim handle re-aims the look-through camera AND sets DAG lookAt; rotate orbits the aim + banks roll == look-through up (`p229-camera-aim-gizmo.spec.ts`; the `up` field added to the `__basher_view_camera` seam observes the bank). KNOWN-LIMIT carried from B-inspector: the `roll` row shows STATIC params during scrub of an animated roll (the H104/#194 read-side caveat — `resolveEvaluatedTransform` returns null for a camera, same as position/lookAt; a camera-transform read-side adapter is the close). Still 2 error patterns at THIS boundary (H67/H69); H118 is a math-primitive input-guard, not a boundary-clustering pattern. The P1 ergonomics tier is complete; next at this boundary would be the camera-transform read-side adapter or #229 polish (single aim-gizmo styling, op coalescing).
**#231 Inc 3 re-validation (2026-06-23) — the boundary now has a MULTI-CAMERA "active" model + GROUPABLE cameras, [[V79]].** Two extensions, NO SHIFT to the instrument/data split (still: editor owns the free orbit camera, DAG cameras are selectable frustums, look-through ADOPTS the active DAG pose). (1) The "active camera" is no longer just the node wired to `scene.camera` — `selectActiveCameraNode` resolves THROUGH a `CameraSelect` (the ClipSelect/V63 switch, active by index, keyframeable → camera CUTS) to the active camera node; the fallback (direct-wired camera) is byte-identical. So EVERY consumer at this boundary that read "the active camera" (look-through `EditorViewCamera`, frustum-marker `SceneFromDAG`, still/anim render, `resolveActiveCameraPoseAt`) now flows through the ONE chokepoint resolver — adding a camera consumer that reads `scene.camera`'s wired node DIRECTLY would see the `CameraSelect` node (not a camera) and break (the new discipline). (2) A camera can be a Group CHILD: `composeCameraPoseWithParent` (`cameraOrientation.ts`, the inverse-bijection that keeps the world pose in the point-based lookAt model) lifts the local pose by the parent world for look-through/still/anim/frustum, the gizmo seeds at world + writes local back (the [[H116]]/#230 round-trip applied to cameras), and `resolveParentWorldMatrix` no longer short-circuits cameras (a node referenced by no `children`/`target` hierarchy socket returns null without the render-root evaluate — the flat-node fast path, generalized). NEW silent-failure modes at this boundary: a camera consumer that bypasses `selectActiveCameraNode` sees a `CameraSelect` not a camera; the world→local write-back guard must mirror the EXACT `childEdges` socket set (`children`+`target`) or a Transform-nested node is wrongly treated as flat; a non-uniform parent SCALE shears the camera basis (aim distance preserved — the documented #230 limit). New observation targets (9): a 2nd camera + Set-Active moves the resolved active camera + the ▲ marker (`p231-active-camera`); a nested camera looks through from the group-composed world + reparents in/out of a Group + the gizmo writes LOCAL params (`p231-nested-camera` — `__basher_view_camera` at world [5,1,0]; gizmo aim world [5,2,0]→local [0,2,0]). Still 2 error patterns at THIS boundary (H67/H69) — the Inc 3 self-review catches (single-socket reorder corruption; guard-must-mirror-childEdges) were caught PRE-commit, not shipped bugs → no clustering escalation. #231 capability B + groupable cameras COMPLETE; next at this boundary = Inc 4 (flatten Scene top-level) or the camera read-side adapter (the carried H104 inspector caveat).

### Boundary B17: Live viewport renderer ↔ offscreen image render (the #168 "Render Image" boundary)

**ORIGIN:** issue #168 (2026-06-05) — the user asked to "produce & download a final image" and a deep e2e of the pipeline. Observed live: the naive `viewportCanvas.toDataURL()` returns a 12 KB BLANK because the live Canvas is `preserveDrawingBuffer:false` ([[H68]]). Surfaced as the second half of the #165 "camera + render" ask.
**WHY:** Without this boundary tracked, the next image-producing feature (animation/sequence render, thumbnail, social-share frame) re-discovers the blank-buffer trap AND independently re-solves camera/resolution/chrome — each a silent-wrong-output class (the call never errors). A render must be a deterministic product of the PROJECT, not the transient editor view; the live front-buffer is the wrong source on all four axes (blank, wrong camera, wrong size, chrome visible).
**HOW:** never read the live front-buffer. `renderToImage.ts` renders the live scene OFFSCREEN into an explicit MSAA + sRGB `WebGLRenderTarget` at `RenderOutput.width/height`, through a production camera built from the active camera node's pose (reuses #165 `cameraPoseFromNode` — independent of the editor orbit view, Blender F12), with editor chrome hidden via the `userData.editorChrome` denylist ([[V32]]), ACES via a temporary `renderer.toneMapping` (restored in `finally`). `renderActiveProjectToPng` orchestrates (threeRef gl+scene out of the Canvas) and downloads via the `exportDag` Blob→anchor idiom. SMAA→4×MSAA is a deliberate, documented divergence (better for a still).
**REF:** issue #168, hetvabhasa [[H68]] (preserve-drawing-buffer blank), [[V32]] (the editorChrome invariant), [[H65]] (the DEV `__basher_render_png` seam discipline), three `WebGLRenderer.js:1273` (MSAA auto-resolve grounding), `src/render/renderToImage.ts` (the offscreen render), `src/app/renderImageAction.ts` (orchestrator + download), `src/app/character/{threeRef,ThreeBridge}.tsx` (gl+scene bridge), `src/nodes/RenderOutput.ts` (width/height + migration), `src/app/{TopToolbar,MenuBar}.tsx` (affordances), `tests/e2e/p168-render-to-image.spec.ts` (decoded-pixel asserts, physically falsified).
**Silent-failure modes:** reading the live `preserveDrawingBuffer:false` canvas → blank PNG, no error ([[H68]]); rendering through the editor orbit camera instead of the production camera → render ≠ the framed shot (post-#165 these are decoupled); rendering at the window/DPI size instead of the explicit resolution → non-reproducible output; editor chrome (grid/gizmo/helpers/fill lights) leaking into the render → not "what production sees"; NOT restoring `renderer.toneMapping`/render-target in a `finally` → the live viewport is left mutated.
**Observation targets:** real-canvas e2e via the DEV `__basher_render_png` seam — decode the PNG and assert (1) it's 1920×1080 ≠ the viewport canvas size, (2) the cube is present (center green, defeats H68 blank), (3) the cube-free bottom-left quadrant is pure background (chrome excluded — physically falsified 0 vs 221 grid pixels), (4) the toolbar button fires a real download named `*-1920x1080.png`. ALWAYS observe the saved PNG (file size + pixel sample + eyeball), never trust "the call returned a data URL." Any new editor-only scene object MUST carry `userData.editorChrome` or it leaks into renders ([[V32]]).
**#12 extension — live render OWNERSHIP (2026-06-15, [[H96]]/[[V52]]):** this boundary now has TWO sides that must stay in lockstep — the offscreen still AND the LIVE viewport, both driven by a postprocessing `EffectComposer` (DoF+SMAA+ToneMapping) from the SAME `cameraDof.ts` settings. The live composer (`PostFx`, imperative, `useFrame(cb,1)`) is the SOLE main-scene render owner; any other priority-positive frame callback MUST overlay-only (the gizmo `<GizmoHelper renderPriority={2}>`). **Silent-failure (it hid for months): drei's `<EffectComposer>` builds ZERO passes here (reads `group.__r3f.objects`, empty in this fiber/drei combo) AND `<GizmoHelper>`/`<Hud renderPriority=1>` re-renders the raw scene over the composer → all post-fx silently gone, no error.** Observation target: log `state.internal.subscribers` priorities in a useFrame — >1 positive-priority main-scene renderer = the bug; and "render nothing + orbit" — if the scene still updates, another renderer owns the frame. Before adding ANY new post-effect (bloom, grade, vignette), confirm PostFx is still the lone priority-1 main renderer. Offscreen DoF parity caveat: the still's composer is 8-bit (UnsignedByte) vs the viewport's HalfFloat — bokeh identical, HDR-highlight tone-map can diverge marginally (documented, like SMAA→MSAA). REF adds: `src/render/PostFx.tsx`, `src/viewport/Viewport.tsx` (`renderPriority={2}`), `src/app/cameraDof.ts`, `tests/e2e/ux12-depth-of-field.spec.ts`.

### Boundary B18: Notification surface ↔ fallible/degradable action callers (the #170/#148 toast boundary)

**ORIGIN:** issues #170 (render "did nothing" with no message — the toolbar/menu `void`-ed the render result) + #148 (boot silently fell back to MemoryStorage → reload lost everything, no warning), 2026-06-05. Built ONE notification surface to serve both ([[H70]]).
**WHY:** Without this boundary tracked, every new fallible/degradable action (export, publish, a future Tauri-fs probe) re-creates a silent no-op: the outcome is REPRESENTED (a `{ ok, reason }`, a chosen fallback) so it looks handled, but no surface consumes it, so the user clicks and nothing happens — diagnosed from scratch instead of matched to H70. It also re-tempts the wrong fixes (throw, or `console.error` — both invisible to the user).
**HOW:** ONE transient surface — `notificationStore` (zustand, V8 UI-projection, idempotent on severity+message, capped, monotonic ids) + `ToastViewport` (App-root mount so it shows in any mode; severity = coloured icon glyph + role/aria-live; component-owned auto-dismiss; `durationMs:0` = sticky). Separate from `assetErrorStore` BY DESIGN — that store is PERSISTENT/asset-keyed (different invariant span); folding them would entangle two lifecycles. Each caller maps its outcome via a PURE function (`renderResultToToast`, `storageFallbackWarning`) and notifies; the action's core return value is preserved. Domain-aligned: one boundary, N callers.
**REF:** issues #170, #148, hetvabhasa [[H70]] (the silent-no-op pattern), [[V33]] (the surface-the-outcome invariant), `src/app/stores/notificationStore.ts`, `src/app/ToastViewport.tsx`, `src/app/renderImageAction.ts` (`renderResultToToast`/`renderImageWithFeedback`), `src/app/boot.ts` (`storageFallbackWarning` + boot notify), `src/app/{TopToolbar,MenuBar}.tsx` (callers), `src/app/stores/assetErrorStore.ts` (the persistent sibling — do not fold), `tests/e2e/notify-surface.spec.ts`, W8 ROWS in `src/a11y/contrastMatrix.test.ts` (the 5 toast token pairs).
**Silent-failure modes:** a caller `void`s a fallible action's result → no toast → silent no-op ([[H70]]); a capability fallback site (`return new MemoryStorage()`, `?? default`) with no adjacent notify → silent degradation; severity conveyed by colour ALONE (fails the icon-glyph accessibility rule); a coloured toast token that skips a contrastMatrix ROW → W8 coverage-gate FAIL; folding transient toasts into the persistent assetErrorStore → entangled lifecycles (one auto-dismisses, the other clears on asset re-render).
**Observation targets:** real-DOM e2e — render from the toolbar → `toast-success` with the resolution; dismiss removes it; boot forced onto Memory (OPFS reject + IndexedDB removed) → sticky `toast-warn` "won't be saved". Falsify by disabling the notify wiring (every toast absent). For any NEW fallible action: "if it returns the failure/fallback branch, how does the USER find out?" — if the answer is console/nothing, wire it through this surface + add a pure outcome→toast mapper test. New toast token colours MUST add a contrastMatrix ROW (audited) — and avoid the literal `border-<word>` / `text-<word>` substring in comments (the gate greps raw file text — a comment phrase can read as a Tailwind class).

### Boundary B19: Spawned overlay ↔ its clipping/placement context (menu flyout / popover / tooltip reachability)

**ORIGIN:** 3+ patterns now cluster at the "a UI surface spawns an overlay that must escape its parent" boundary — [[H91]] (stale placement anchor), [[V45]] (flyout placement vs the LIVE viewport via the shared `useFlyoutSide`), and [[H103]] (an `overflow-hidden` ancestor silently CLIPPED every MenuBar submenu flyout out of existence; the user reported "can't reach any submenu", 2026-06-17). Promoted to a boundary because the hetvabhasa clustering IS the organizational signal.
**WHY:** Without this boundary tracked, each overlay surface (menu submenu, AddMenu group, a future inspector popover/tooltip/context-menu) re-discovers the same class one at a time: an overlay that opens BEYOND its container fails in a way that DOM-presence tests pass over (it's mounted) while real users can't see or click it. The trap is to chase the event handlers (hover/click/timing) when the surface is simply clipped or mis-placed and not hit-testable.
**HOW:** Two rules. (1) PLACEMENT: every spawned overlay computes its side against the live viewport through the ONE shared primitive (`useFlyoutSide`), never a hardcoded side/edge constant ([[V45]]). (2) ESCAPE: an overlay that extends past its parent CANNOT live inside an `overflow:hidden` ancestor — that chain clips it to nothing (invisible + not hit-testable). Use `overflow-visible` on the panel/flyout (MenuBar) or portal the overlay out. Hover-open overlays also need: click OPENS (never toggles-shut what hover opened) + a close GRACE delay cancelled on re-entry (the trigger→flyout seam).
**REF:** [[H103]] / [[H91]] / [[V45]], `src/app/MenuBar.tsx` (Menu panel + Submenu flyout `overflow-visible`, `SUBMENU_CLOSE_DELAY_MS`, onClick `openNow`), `src/app/menu/useFlyoutSide.ts` (the `left:100%` placement that opens beyond the panel), `src/app/AddMenu.tsx` (`AddMenuGroup`, same primitive), `tests/e2e/ux5-submenu-reachable.spec.ts` (real-pointer regression), `tests/e2e/ux5-menu-submenu-edge.spec.ts` (placement).
**Silent-failure modes:** an `overflow-hidden`/`clip`/`scroll` ancestor clips an overlay that opens outside its box → the overlay is in the DOM but invisible + hit-tests to whatever is behind it ([[H103]]); a hardcoded `left-full`/`right-0` placement runs the overlay off-screen near a viewport edge ([[V45]]/[[H91]]); a hover-opened flyout's trigger click toggles it shut; no close-delay → the trigger→flyout seam closes it mid-reach. The unifying tell: `count()`/`toBeVisible()`/Playwright `.click()` are GREEN while the human can't reach it.
**Observation targets:** the decisive probe is `document.elementFromPoint(overlayCenterX, overlayCenterY)` — it must return the overlay (or a descendant), NOT an ancestor/sibling/`<canvas>`. Bounding-box presence is NOT reachability. Regression e2e must drive REAL `page.mouse.move`/`down`/`up` (Playwright's `.hover()`+`.click()` teleport and mask both clipping and the seam). For any new overlay: open it, `elementFromPoint` its center, assert it's the overlay; then walk ancestors for `overflow` ≠ visible while the overlay's rect sits outside that ancestor's box.
...
**W7 trigger (a) — RESOLVED #57 (2026-06-02).** The "R8 + ModeBadge over the rendered viewport, dark + light scenes both pass §8.4" trigger was acknowledged-but-not-literally-satisfied at D-W8-1 (the matrix composited only against `bg #0a0a0a` — the dark scene). #57 closes it by observation: [[H62]] catalogues the trap (an opaque-only audit composited against the BEST-case fixed bg silently passes translucent chrome over a VARIABLE backdrop). The light-scene half is now gated two ways — a worst-case-`#ffffff` recomposite `it()` in `contrastMatrix.test.ts` (formula) + `tests/e2e/p57-bright-scene-contrast.spec.ts` driving the REAL GL canvas bright via the DEV-only `__basher_setSceneBackground` seam and pixel-sampling the composited overlay (observation), with a falsification guard that the bright scene actually moves the surface. Result: `bg-2/90` masks the canvas, worst idle glyph ≥5.44:1 (formula) / 5.47:1 (measured) over `#ffffff` → aesthetic kept, no surface change. **New observation target at this boundary:** any future chrome that mounts over the GL canvas (or any media/themed backdrop) with a `/N`-alpha bg MUST be audited against the worst-case backdrop, not the page-bg token — recomposite against `#ffffff` AND pixel-sample over a bright scene. This is now a permanent CI gate, not a per-wave manual check. ORIGIN: #57 (last v0.5 W8 follow-up). WHY: without it, the next over-canvas overlay inherits the same silent-pass blind spot the matrix's green gate creates. HOW: the [[H62]] two-layer recipe (formula worst-case row + real-pixel e2e + falsification). REF: [[H62]], `src/a11y/contrastMatrix.test.ts`, `tests/e2e/p57-bright-scene-contrast.spec.ts`, `src/viewport/SceneBgTestSeam.tsx`, `docs/UI-SPEC.md` §8.4.1.
...
**UX #11 re-validation (2026-06-15, dopesheet rebuild + shared-view "unify", commits `b3af019`→`b1c90db`):** re-ran B11 HOW over the W5/W9 timeline distinctness pairs after the reze-fidelity dopesheet + the shared zoom/pan slices.

| Pair (from W5/W9 inventory)                         | UX #11 result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TimelineCanvas (dopesheet) ↔ CurveEditor (curve)    | **NO SHIFT to the SPLIT, but a NEW SHARED subscription.** Still disjoint domains (canvas all-channels vs SVG one-channel) + different primitives + different playhead impls (rAF-bypass canvas vs React SVG line) — D-UX-2/D-UX-3 hold. BUT they NOW BOTH subscribe to `timelineViewStore` (zoom/scroll) — the W5 "different store subscriptions" sub-claim is **advanced**: the time-AXIS is deliberately shared (the user-chosen "unify" = shared view, NOT merged surface; AskUserQuestion 2026-06-15) while the channel-DATA subscriptions stay disjoint. Recorded as [[V50]] (one shared `timelineView`; default-view === keyframeToRect parity is what kept p7.1/p7.12/p6-w9 green through the refactor). |
| currentFrameRef ↔ timeStore.frame (rAF playhead)    | **NO SHIFT.** The playhead still reads `currentFrameRef`/`seconds` via getState in the rAF (zero React render); it now ALSO reads `timelineViewStore.getState().view` the same way (single source, no closure capture). The dual-channel single-source invariant + the p7.12(e) perf guard (commits===0 during playback) both hold. Auto-scroll deliberately DEFERRED precisely because continuous `setView` from the rAF would repaint the static-layer cache per frame (the D-W9 bottleneck the cache exists to avoid) — if added, must page-jump on threshold, not scroll continuously.                                                                                                                      |
| static-layer cache ↔ playhead-strip-redraw (W9 (d)) | **NO SHIFT, strip widened.** The glow (slice 2) widened the strip-restore to `PLAYHEAD_GLOW_HALF_PX`; the static layer now also repaints on view change (zoom/pan). Strip restore still covers the moving playhead+glow (no trail); the static cache is the offscreen twin restored 1:1. No desync.                                                                                                                                                                                                                                                                                                                                                                                                             |

New trap catalogued: **[[H95]]** — the canvas/SVG-drag e2e (p7.1, p7.12) MIRROR TimelineCanvas's geometry constants (gutter/diamond/row-height/ruler); a geometry change silently mis-targets them until updated in lockstep (slice 1 hit both). Durable cure: tests import a shared geometry helper ([[V50]]'s `timelineView`) instead of copying constants. **UX #11 re-validation triggers:** (a) if box/multi-keyframe select lands → `timelineSelection` grows from one `activeKeyframeId` to a set → re-inventory the single-key selection sub-claim across dopesheet+curve+toolbar; (b) if the read-only clip-row curve path (CurveEditor's `clip:` viewBox branch) is migrated onto the shared view → re-validate it against [[V50]]; (c) if auto-scroll lands → re-inventory the static-cache↔view-write boundary for the per-frame-repaint risk above.
...
**UX-BACKLOG #2 section-inventory pass (2026-06-14) — floating-island layout.** The chrome-LAYOUT changed, not the chrome SECTIONS: the outliner / inspector / agent chat / timeline stopped being grid columns/rows and became absolute islands floating over a full-bleed viewport (slices 1+2). Ran B11 HOW (re-validation) over every surface the change touches — **files OPENED, not recalled** (H25/H27):

| Surface (file:line opened)                                | What changed (LAYOUT only)                                                                                                 | Distinctness / collision?                                                                                                                               |
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Outliner `src/app/LeftSidebar.tsx:104,82` + `Layout.tsx`  | grid `tree` column → absolute top-left island; inner `<aside>` bg→transparent, border-r dropped (wrapper owns the surface) | NO section change. Tabs (Outliner\|Assets), search, tree intact. V35 reveal preserved (collapsed strip + expand chevron).                               |
| Inspector `src/app/NPanel.tsx:1346,1324`                  | grid `inspector` column → absolute top-right island; inner bg→transparent, border-l dropped                                | NO section change. Selection-driven sections + collapse chevron intact.                                                                                 |
| Agent chat `src/app/AgentDock.tsx:25` + `Layout.tsx`      | grid `agentdock` row → bottom-center island (top of the stack); inner bg→transparent, border-t dropped                     | NO behaviour change (§196 holds — same AgentChat). Now a centered capped-width island, not full-width.                                                  |
| Timeline `src/timeline/TimelineDrawer.tsx` + `Layout.tsx` | grid `timeline` row → bottom-center island (below the chat); Timebar (Auto-Key REC) stays always-visible (D-UX-14 footgun) | NO section change. Drawer reveal + Timebar split unchanged; the always-visible-Timebar invariant holds (island always mounted, hidden only in present). |
| Toolbar `src/app/FloatingViewportToolbar.tsx:283`         | width-capped (`CENTER_SIDE_RESERVED`) so a viewport-centered pill can't slide under a side island                          | NO section change; already a floating island (the precedent). New geometry constraint only.                                                             |
| Orbit gizmo + Persp/Ortho + orbit hint (`Viewport.tsx`)   | side islands stop short of `BOTTOM_BAND` so these stay clear; orbit hint moved above the bottom stack                      | NO collision — the reserved bottom band keeps the bottom-right widgets and the bottom-center stack physically separated.                                |

**VERDICT: NO SHIFTS.** No D-UX distinctness claim decayed; this is a layout relocation (grid→absolute islands), not a section merge/split. New invariant **[[V46]]** captures the floating-island geometry contract (one geometry source; reserve a clear region for every co-floating surface; the [[H91]]/[[V45]] overlap family generalized to the whole layout). ORIGIN: UX-BACKLOG #2, the user-committed floating-panel redesign; WHY: a full-bleed viewport with absolute islands has NO grid to reserve space, so overlap is the default failure and needs an explicit invariant + e2e gate; HOW: `layoutIslands.ts` single source + `ux2-floating-islands.spec.ts` falsifiable gates + the per-surface table above.
...
**B12 baked-material-capture extension 2026-06-04 (#151 / phase 151 Apply-Transform):** the render-consumer surface now spans an EIGHTH concern — the boundary now also PRODUCES a persisted BAKED material, not just a live override. When a director Applies a glTF child, `captureBakedMaterial` reads the RESOLVED, POST-override material off the live mounted clone at the SAME GltfAssetR seam B12's #99/#124 override effect writes (`SceneFromDAG.tsx:732-802`), and persists each texture map to OPFS (self-contained content-hashed bytes, [[V30]]) — the bake-what-renders capture point is the same resolved-not-raw discipline B12 established for the override ([[H58]]/[[H59]]). Two B12 facts this pins: (1) **the capture is READ-ONLY at the clone boundary** — it reads scalars + copies texture bytes, NEVER mutates the live clone material (the clone material is already a per-instance `s.clone()` #99; a mutating capture would be the [[V20]]/[[H36]]/[[H45]] single-writer landmine — the SAME landmine B12 catalogues, now on the capture side). (2) **the clone is accessed via the production-safe `gltfCloneRegistry`, NOT the DEV `__basher_gltf_meshes` seam** ([[H65]]) — the new accessor mirrors B12's existing live-clone seams (`__basher_gltf_skin`/`__basher_gltf_meshes`) but is always-on, so Apply does not silently no-op in the production build. **This is where B12 (material) meets B14 (mesh resolver):** the BakedMesh material band is captured at B12's seam and consumed at B14's resolver — the two boundaries are now coupled at the bake. **Fatality note:** this is a FOURTH concern at the loaded-glTF clone boundary (skeleton-rebind H45, material-drop H59, override-tracking #124, now bake-capture) — but all are the SAME clone-semantics family ("clone before mutate / read-not-write the shared loaded resource"), enforced by the ONE [[V20]]/[[H45]] single-writer + clone-first rule, NOT 4 independent root causes. No new fatality; the clone boundary stays one well-placed seam. **REF:** `src/app/animate/captureBakedMaterial.ts` (the read-only POST-override capture), `src/app/asset/bakedTextureStore.ts` (`persistTexture` self-contained byte copy), `src/app/asset/gltfCloneRegistry.ts` (the production-safe clone access), `src/viewport/SceneFromDAG.tsx` GltfAssetR (the capture seam = the override seam) + BakedMeshR (the consumer), `tests/e2e/p151-gltf-child-apply.spec.ts` (SC-6 lossless material reload + the bake-what-renders-with-a-pre-existing-override gate), [[V30]] [[H45]] [[H58]] [[H59]] [[H65]] [[V20]] [[H36]] [[H40]], CONTEXT/PLAN 151. Issue #151. (Grounded: GROUND_TRUTH_GLTF.md @ three 0.169.0 — materials §STAGE 3, clone/share boundary §STAGE 6.)

### Boundary B15: glTF live-clone production accessor (`gltfCloneRegistry` — mounted clone ↔ non-React production helpers)

**ORIGIN:** #151 Wave 4 (2026-06-04, Apply-Transform). The crux: `dispatchApplyTransform` — a non-React production helper — needs the mounted SkeletonUtils clone of a GltfAsset to read a child's resolved geometry + post-override material (`geometryRegistry.get()` returns null for gltf; the bytes live in the loaded asset). The OBVIOUS reach was the existing live-clone accessors `window.__basher_gltf_meshes`/`__basher_gltf_skin` — but those are `import.meta.env.DEV`-gated OBSERVATION seams, so reaching through them would make Apply a SILENT NO-OP in the production build ([[H65]]). A SEPARATE, always-on production accessor was needed.

**WHY:** without this boundary tracked, the next production feature that needs live three.js render state (a future export, a snapshot, a second bake variant, a viewport-side measurement) reaches through the convenient DEV seam and ships a silent prod no-op no e2e catches (the dev server runs the DEV build, so the seam is present in every test). The class of problems made invisible: "a shipping code path depends on a development-only surface." Removing this entry reopens "diagnose the next silent-prod-no-op from scratch instead of matching the DEV-seam-in-prod-path pattern."

**HOW (the focus + observation targets):**

- The boundary is **a production-safe live-clone accessor**: `src/app/asset/gltfCloneRegistry.ts` — a module-level `Map<assetRef, THREE.Group>` that `GltfAssetR` registers on mount + on every clone swap and unregisters on unmount (newest-mount-wins guard so a late unmount can't clobber a re-registered ref; matches the single-asset-per-assetRef assumption the DEV seams already make). Consumers: `dispatchApplyTransform` (reads the clone to clone-child-geometry + capture-resolved-material). It MIRRORS the DEV getters' access pattern (same clone, same by-name walk) WITHOUT the `import.meta.env.DEV` gate.
- **Silent-failure modes:** (a) [[H65]] — a shipping path reads a `__basher_*` DEV getter → `undefined` in prod → silent early-return; observation target = grep `src/` (excluding `*.test.*`) for `__basher_` in non-test production code. (b) a stale registry entry (asset unmounted but not unregistered) → `getGltfClone` returns a dead clone; gated by the newest-mount-wins + identity-checked unregister. (c) the clone read is a SNAPSHOT (not subscribed) — correct here because `dispatchApplyTransform` is a one-shot user action, NOT a render effect (the [[H40]]/B12 subscribed-selector rule applies to RENDER consumers, not to one-shot dispatch reads — do not confuse the two).
- **Fatality re-test (2026-06-04):** does this new boundary cluster with B12's clone boundary? It is ADJACENT (same mounted clone) but a DISTINCT concern: B12 = how the clone's material/skeleton is BUILT + written (single-writer); B15 = how a PRODUCTION HELPER ACCESSES the clone (DEV-gate-free). One hetvabhasa ([[H65]]) lives here so far — not clustering. Tracked separately so the access-pattern trap is matched deductively next time.
- **REF (grounded):** `src/app/asset/gltfCloneRegistry.ts` (the accessor + the comment naming the H65 trap it avoids), `src/viewport/SceneFromDAG.tsx` GltfAssetR (the populator — `registerGltfClone`/`unregisterGltfClone` on mount/unmount + the mirrored DEV seams), `src/app/animate/dispatchApplyTransform.ts` (the consumer), `tests/e2e/p151-gltf-child-apply.spec.ts` (the real-render proof Apply works on the live clone). Enforces [[H65]] · sibling of [[B12]] (the clone boundary it accesses) · [[H58]] (unreachable-by-users twin). Issue #151.
...
**B13 — EDIT-time (not playback-time) re-render, FIXED 2026-06-11 (`02f04a1`, `ux-overhall`).** P7.10 killed the PLAYBACK re-render (time-subscription → 0 commits). A distinct EDIT-time leak remained at the same boundary: `GltfAssetR` subscribed to the WHOLE node table (`useDagStore((s) => s.state.nodes)`), whose ref flips on every dispatch (`ops.ts:278-282`), so editing ANY node re-rendered every mounted glTF asset + re-walked all N nodes twice → ~16fps on a ~700-node import. The `SceneChildNode` value-ref memo (52a3958) closes the PARENT re-render path but cannot help a `useSyncExternalStore` re-render — a leaf's own broad subscription re-opens the cost ([[V42]]). Fixed by narrowing the subscription to this asset's dep nodes (`gltfAssetDepNodes`) with `shallow`. **A prior session named the WRONG lever here** (evaluator `hashValue` cache-miss) — an inference off the false-witness `__basher_evaluate` seam; a unit probe falsified it (the value ref was already stable, same hash across an unrelated edit). See [[H48]] 5th occurrence.

**Observation target (EDIT-time, NEW — complements the playback-time profiler above):** the playback gate (`perf-fox-benchmark`, `react.p95` + `commits`) does NOT cover edit-time leaks — it never edits a node. For edit-time, use the render-count seam: `bumpRenderCount` (`src/perf/renderCounter.ts`) + `window.__basher_render_counts()`, asserted by `tests/e2e/perf-render-count.spec.ts` (edit a SIBLING 10× → the heavy renderer fires +0; own-node edit +1). **Trap (this boundary's signature failure):** a render-count proves WHICH component re-renders, NOT WHY — it cannot tell "parent re-rendered me" from "my own subscription fired" from "cache missed." Isolate the path with a control (unit value-ref probe + self-edit vs unrelated-edit render-count split) BEFORE naming a lever. **REF:** `src/app/gltfAssetDeps.ts`, `src/viewport/SceneFromDAG.tsx` GltfAssetR (narrowed `useStoreWithEqualityFn`), `src/perf/renderCounter.ts`, `tests/e2e/perf-render-count.spec.ts`, `src/app/gltfAssetDeps.test.ts`, [[V42]] [[H48]] [[H40]]. Commit `02f04a1`.

**B13 — CORRECTION (2026-06-12, [[H48]] 6th occ): the render-count target above is NECESSARY BUT INSUFFICIENT; it gates the mechanism, not the fps.** An end-to-end frame-budget A/B on the real cicada (headed Firefox) proved the subscription fix removes re-renders (59→0) but does NOT move frame time (43.2ms→43.8ms). The render-count seam tells you a component stopped re-rendering; it does NOT tell you the frame got faster. **Upgraded observation protocol for THIS boundary:** (1) render-count seam → mechanism gate (a component re-renders 0×); (2) **`__basher_perf` frame.mean A/B (old code vs new) → the ONLY proof a fix moved fps;** (3) component microbench (time each sub-op on a synthetic N-node tree) → WHERE the time is, BEFORE touching code. Lead with (3) then (2). The cicada's actual edit-lag bottleneck is NOT this boundary — it is the GIZMO/selection path (B-gizmo, below). **Measurement infra (hard-won):** headed **Firefox** (Chromium OOMs the 100MB cicada at import AND floods the CDP pipe with per-frame PostFx `glBlitFramebuffer` GL warnings → page closes); write the full playwright log to a FILE (a `| grep` pipe block-buffers and hides interim/result lines); `pkill -9 -f firefox` between runs (crashed headed runs leak processes that thrash the next run into a timeout); macOS has no `stdbuf`.

### Boundary B-gizmo: the selection/manip per-frame cost — RESOLVED 2026-06-12 ([[H48]] 7th occ). Root cause was NOT the gizmo; it was `hashValue(node.params)` re-hashing a heavy import on every uncached `evaluate`.

ORIGIN: 2026-06-12 cicada bottleneck hunt ([[H48]] 6th occ). The "manipulate a node → 120→2fps" the user reported.
WHY (kept as a standing trap): the 6th occ's framing — "a mounted gizmo costs ~117ms/frame, the bulk is the gizmo path" — was a MEASUREMENT ARTIFACT. The 7th-occ bisection on the REAL (rendering) cicada FALSIFIED it: disabling `<TransformControls>` changed nothing (510/594ms either way); the cost is `resolveEvaluatedTransform` → uncached `evaluate(render root)`/`evaluate(GltfAsset)` (35.7ms each), called per inspector transform row + StrictMode → ~458ms/commit. The +117/+414 split and "non-React" attribution came from (1) the React `<Profiler>` budget covering ONLY the Canvas subtree, so the DOM inspector's (NPanel's) re-render cost read as "non-React frame time," and (2) the gizmo's seeding `useEffect` ALSO calling the resolver. Tracking the WRONG boundary (the gizmo/TransformControls) is what this entry now guards against.
HOW (the resolution + the durable observation protocol): root cause = the evaluator re-hashes every node's params on every uncached evaluate (evaluator.ts:127, paramsHash is the cache KEY, computed BEFORE the lookup → a warm cache doesn't help). FIX = WeakMap memo of `hashValue(node.params)` by params identity ([[H48]] 7th occ, commit `6fa544a`) → evaluation O(changed); edit-lag 590→14ms. **The protocol this boundary now encodes for any future "heavy-import is slow":** (1) measure on the asset that ACTUALLY RENDERS — write its bytes to OPFS or use an `http://` passthrough assetRef so `useGLTF` draws it; a buffer-only import builds the DAG but renders nothing and a synthetic understates (its GltfChild nodes aren't wired into the render tree). (2) Bisect with falsifiable controls (gizmo on/off; mesh-count vs tri-count vs material-count fixtures; selection×edit decomposition) — do NOT trust the prior session's named lever. (3) Remember the `<Profiler>` budget excludes DOM chrome — "non-React frame time" can be a React inspector re-render. Files: `src/core/dag/evaluator.ts` (`hashParams`/`paramsHashMemo`), `src/app/resolveEvaluatedTransform.ts`/`resolveTransformParam.ts`/`NPanel.tsx` (resolver callers). Seams: `__basher_perf` (frame/react), `__basher_writeOpfsBytes`, `__basher_importGltf`, `__basher_selection`. REF: [[H48]] 7th occ, [[B13]], [[V42]], `src/core/dag/paramsHashMemo.test.ts`. Commit `6fa544a`.

### Boundary B14: Evaluated-mesh boundary (`resolveEvaluatedMesh` — one producer, many consumers) — IMPLEMENTED (v0.6 #1 / #150, 2026-06-03)

**ORIGIN:** v0.6 #1 unified-mesh-model planning (2026-06-03, issue #150, phase `v06.1-unified-mesh-model`). The §58 invariant "see/edit ANY model's geometry, UVs, materials, transform" is split across four islands (`BoxMesh`/`SphereMesh`/`GltfAsset`/`GltfChild` value kinds; TRS for glTF but position+rotation-only for primitives; two material engines; UVs generated for primitives, not extracted for imports). The phase introduces ONE `EvaluatedMesh{geometry, uvs, material(PBR), transform(TRS)}` produced by a pure `resolveEvaluatedMesh(node, ctx)` and consumed uniformly by gizmo + inspector (then material #2 / UV #3 / transient-keyframing #149). This is the SAME shape as [[B1.1]] (the #68 gizmo-proxy↔evaluated-scene boundary-pair) generalized from transform to the whole mesh.

**WHY:** without this boundary tracked, the next consumer (material editor, UV editor, the #149 transient-keyframe band) re-branches on glTF-vs-primitive and the four islands leak further; and a new EvaluatedMesh band (scale now; material/uv later) that is threaded into the read-side resolver but NOT the renderer (or vice-versa) reopens the [[H40]] displayed-≠-rendered class at mesh scope. Removing this entry means each rider re-discovers the one-producer-many-consumers discipline from scratch.

**HOW (the focus + observation targets) — GROUNDED (Wave 5, 2026-06-03):**

- `resolveEvaluatedMesh` (`src/app/resolveEvaluatedMesh.ts`) is the SOLE producer; the renderer (`src/viewport/SceneFromDAG.tsx` BoxMeshR `scale={value.scale ?? [1,1,1]}` + SphereMeshR, mirroring TransformR's `<group scale>`) and EVERY read-side surface (Gizmo `getManipulable` `scaleParamPath`→'scale', NPanel via `resolveTransformParam` 'scale' whitelist) apply the SAME band at the SAME `ctx.time`. For GltfChild the resolver DELEGATES the transform to `resolveEvaluatedTransform` → the one `resolveGltfChildTrs` band — no parallel walk (Chesterton, the "one band, two callers" BLOCK-1 rule, [[B1.1]] template).
- **Silent-failure modes (all gated):** (a) band-in-resolver-not-renderer → gated by the side-A/side-B equality e2e below; (b) hydrate-seam read of a post-release field without a `?? default` → gated by the C-1 / [[V10]]/[[H14]] two-layer `?? [1,1,1]` guard at the evaluator (`BoxMesh.ts`/`SphereMesh.ts` evaluate) AND every consumer (BoxMeshR/SphereMeshR, `resolveEvaluatedMesh.ts`); (c) schema migration changing an existing project's render → gated by IDENTITY default in zod `.default` + `migrations[1]` + evaluator, proven byte-identical by `src/core/project/migrations.test.ts`.
- **Observation target (the per-consumer sibling-spec rule):** EVERY new mesh consumer (material #2, UV #3, transient-keyframe #149) lands a sibling boundary-pair spec that reads the REAL rendered three.js object value (scene-walk seam) AND the resolver value and asserts equality — never infer from node params ([[H58]]). Grounded seams: side A `__basher_mesh_world_scale` (`SceneFromDAG.tsx` `MeshScaleProbe`, scene-walk by node-id-named group, mirrors `__basher_gltf_meshes`, C-3); side B `__basher_evaluated_mesh` (`src/app/boot.ts` → `resolveEvaluatedMesh`).
- **Fatality re-test (2026-06-03):** does the evaluated-mesh boundary cluster 3+ hetvabhasa? **NO** — it inherits [[H40]]'s single well-placed boundary (the mesh resolver is H40's THIRD catalogued consumer, not a new failure mode). Vyapti span is ALIGNED: the invariant ([[V29]]) is enforced in ONE module (`resolveEvaluatedMesh`). No consolidation needed; the structure matches the domain.
- **REF (grounded):** `src/app/resolveEvaluatedMesh.ts`, `src/app/geometryRegistry.ts`, `src/nodes/types.ts` (EvaluatedMesh/GeometryRef/MeshTransform), `src/viewport/SceneFromDAG.tsx` (BoxMeshR/SphereMeshR scale + `MeshScaleProbe`), `src/nodes/{BoxMesh,SphereMesh}.ts` (v2 + migration), `src/app/Gizmo.tsx`, `src/app/boot.ts` (`__basher_evaluated_mesh`), `src/core/project/migrations.test.ts`, `tests/e2e/p150-evaluated-mesh.spec.ts` + `p150-uniformity-gate.spec.ts` + the migrated `gizmo-autokey-record.spec.ts`, `.planning/phases/v06.1-unified-mesh-model/{CONTEXT,RESEARCH,PLAN}.md`. Mirrors [[B1.1]] · enforces [[V29]] · [[H40]] [[H36]] [[V20]] [[V10]]. Issue #150 (foundation); riders #149/#151/#2/#3.

**B14 #151 extension 2026-06-04 (#151 / Apply-Transform) — the FOURTH producer `BakedMesh`, the first AUTHORITATIVE-store producer, and the first registry-READING renderer.** The evaluated-mesh boundary now spans a producer whose geometry is NOT rebuildable-from-params. `BakedMesh` is the 4th `resolveEvaluatedMesh` branch (after Box/Sphere/GltfChild), returning `{geometry: the baked GeometryRef verbatim, uvs:null, material: the captured BakedMaterialSpec, transform: identity}`. Three boundary facts this extension pins:

- **AUTHORITATIVE not derived ([[V30]]):** box/sphere geometry is a DERIVED V1-exempt registry cache (rebuilt on demand); a baked `applyMatrix4` result + captured textures are AUTHORITATIVE bytes — they persist to OPFS keyed by content hash, the DAG carries only the handle. This SHIFTS the geometryRegistry V1-EXEMPTION boundary that B14 previously treated as uniform: the registry now has TWO classes (derived box/sphere = rebuild; baked = OPFS-load). The observation target gains: a baked producer's bytes MUST survive reload (SC-3) and a deleted source asset (the self-contained texture copy, [[H60]] avoided).
- **First registry-READING renderer ([[H40]] new guise):** `BakedMeshR` is the FIRST renderer to consume `geometryRegistry.get()` via the `useBakedGeometry`/`useBakedTexture` suspense hooks (Box/SphereR build inline) — the §48/V29 handle→registry→buffer path comes alive here. It renders IDENTITY scale (the TRS is baked into the verts; re-applying would double-transform — the band-drift guard). The per-consumer sibling-spec rule held: `tests/e2e/p151-apply-transform.spec.ts` + `p151-gltf-child-apply.spec.ts` assert RENDERED world bounds == resolver geometry bounds == pre-bake world bounds (three-way boundary-pair).
- **Capture is bake-what-renders ([[B12]] tie-in):** for a glTF child the material is captured POST-override off the live mounted clone (via `gltfCloneRegistry`, NOT the DEV seam — [[H65]]) — the SAME resolved-not-raw discipline B14's transform band uses, now extended to the material band. This is where B14 meets B12 (see the B12 material extension below).
- **Fatality re-test (2026-06-04):** does adding the 4th producer + the authoritative-store split cluster 3+ hetvabhasa at the evaluated-mesh boundary? **NO.** The new traps are the EXISTING families' extensions: [[H40]] (band-drift, 4th catalogued consumer), [[H45]] (clone-before-mutate, the bake instantiation), [[H65]] (a NEW entry but at the gltfCloneRegistry boundary, not the mesh-resolver boundary — see its own dharana entry below). [[V30]] lands ALIGNED: the authoritative-store invariant is enforced in ONE module pair (`bakedGeometryStore`/`bakedTextureStore`) consumed by the registry + the renderer hook. No consolidation needed; the structure matches the domain (the registry's two-class split mirrors the domain's derived-vs-authoritative distinction).
- **REF (#151):** `src/app/resolveEvaluatedMesh.ts` (4th branch), `src/app/geometryRegistry.ts` (`'baked'` branch), `src/app/asset/bakedGeometryStore.ts` + `bakedGeometryLoader.ts` + `bakedTextureStore.ts` + `bakedTextureLoader.ts`, `src/nodes/BakedMesh.ts`, `src/viewport/SceneFromDAG.tsx` BakedMeshR (+ `__basher_mesh_world_bounds`/`__basher_baked_geometry_bounds` probes), `src/app/animate/dispatchApplyTransform.ts`, `tests/e2e/p151-apply-transform.spec.ts` + `p151-gltf-child-apply.spec.ts`, `.planning/phases/151-apply-transform/{CONTEXT,RESEARCH,PLAN}.md`. Enforces [[V30]]/[[V29]] · [[H40]] [[H45]] [[H65]] [[K15]] · ties to [[B12]] (material capture). Issue #151.

**B1.1 + B14 #149 extension 2026-06-04 (#149 / Transient Edit + Keyframing) — the transient overlay is a NEW BAND on the existing resolver boundary, no new boundary; FLAG-A's reject alert is SUPERSEDED by the orange indicator.** Editing an ANIMATED param with Auto-Key OFF now HOLDS the edit as a transient ([[V31]]) instead of rejecting it. The transient is overlaid by the ONE shared `overlayTransients` primitive at BOTH the render side (`AnimationLayerR`, inside the sole TRS writer — the [[B1.1]] producer) AND the read side (`resolveEvaluatedTransform` for transform + the NET-NEW generic `resolveEvaluatedParam` for non-transform — the [[B1.1]]/[[B14]] consumers). This is the SAME "one band, two callers" structure both boundaries already enforce — the transient is a NEW BAND (transient > channel precedence), not a new boundary. Facts this pins:

- **Two NEW [[H40]] drift forms, both gated by the PAUSED boundary-pair e2e** (the observation target this boundary always demanded — observe BOTH sides against the REAL three.js object, paused): form 1 = the read resolver re-implements interpolation instead of `.sample()` (grep-gated + C4 non-transform e2e); form 2 = the render dirty-check omits the transient ref → paused-edit freeze (subscribed selector + transient in the lastApplied tuple + C3 transform e2e). Both OBSERVED green: rendered `[9,0,0]`/`#808080`/`#ff0000` == resolver == transient.
- **FLAG-A supersession (the Chesterton resolution):** the removed `window.alert` reject at `autoKeyCommit.ts` (it replaced the #68/#77 silent no-op) is NOT deleted into silence — it is replaced by the **orange field color** (`text-warn`, TOP of the diamond precedence, [[V31]]/F1) plus the viewport overlay (the held edit moves the object from Wave B). The milestone-close gate was: orange exists before merge. Met.
- **H48 perf safety:** `AnimationLayerR` gains its FIRST subscribed store selector (`useTransientEditStore(s=>s.edits)`) in this perf-sensitive path — SAFE only because transients are paused-only + `clearAll` no-ops when the set is empty, so `commits=0` over playback (OBSERVED on perf-fox, 2/4 foxes, react.p95 0.00). The "no time SELECTOR" [[H48]] rule still holds (seconds read as a snapshot).
- **Fatality re-test (2026-06-04):** does the transient band cluster 3+ NEW hetvabhasa at the resolver boundary? **NO.** Both new drift forms are [[H40]] extensions (same family, now the 4th catalogued consumer = transient-overlay). [[V31]] lands ALIGNED — enforced in ONE primitive (`overlayTransients`) + one store, the two callers thin. No consolidation needed; the structure matches the domain (the transient is one band overlaid by one primitive at two sites).
- **REF (#149):** `src/app/stores/transientEditStore.ts`, `src/app/overlayTransients.ts`, `src/app/animate/autoKeyCommit.ts` (`routeAnimatedGrab` OFF branch + `keyParamFromTransient`), `src/viewport/SceneFromDAG.tsx` (AnimationLayerR overlay + dirty tuple + `__basher_mesh_world_position`), `src/app/resolveEvaluatedTransform.ts` + `src/app/resolveEvaluatedParam.ts`, `src/app/NPanel.tsx` (orange + diamond commit), `src/app/KeyboardShortcuts.tsx` (K/I), `src/app/animate/dispatchMutator.ts` (`dispatchFirstKeyComposite` layer-reuse), `tests/e2e/p149-{transient-boundary-pair,clear-on-scrub,commit,four-color}.spec.ts`, `.planning/phases/149-transient-edit-keyframing/{CONTEXT,RESEARCH,PLAN}.md`. Enforces [[V31]] · [[H40]] [[H36]] [[V20]] [[V29]] [[H48]] · ties to [[B1.1]] (gizmo proxy now overlays the transient too). Issue #149. (Grounded: `~/.anvideck/projects/basher/ref/GROUND_TRUTH_BLENDER_KEYING.md`.)
...
**OBSERVED RESULT — flatten IS a separate coarse boundary (#131 Wave E SHIPPED, PR #134 MERGED `f443e47`, 2026-06-02):** `ignoreSourceMaterial: z.boolean().default(false)` landed on `MaterialOverride`, riding the same `override?: MaterialValue` prop chain to `GltfAssetR`. It confirmed the design prediction (D-05 / line 1273): flatten is a DIFFERENT-SCOPE primitive, NOT folded into the per-field `overridden` set. In the renderer it is a clean branch — `true` builds a FRESH `MeshStandardMaterial` from the 7 scalars (drops the source maps + subclass BY INTENT, the honest opt-in version of the old #99 wholesale-replace bug); `false` keeps the #99/#124 clone + map-aware-merge path verbatim. The per-field set is irrelevant in flatten mode (the two primitives do not interact). **Single-writer held ([[V20]]/[[H36]]/[[H45]]):** flatten still re-derives from the lazily-captured original and assigns a fresh per-mesh material — it never mutates the shared `useGLTF` source, and toggling off restores every map. GOAL observed live (Lokayata, H40/H59 boundary-pair): `tests/e2e/p131-material-flatten.spec.ts` flips `ignoreSourceMaterial` on a real textured-metal asset → `__basher_gltf_meshes()` reports `hasMap`/`hasMetalnessMap`/`hasRoughnessMap` all `false` (maps gone by intent) and the clay scalars land; off → every map restored. The clone path (#99/#124) is proven untouched by p7.13 + p124 green.

**OBSERVED RESULT — the decorator fixes BOTH consumers as predicted (#130 Wave D SHIPPED, 2026-06-02):** the consolidation dividend held. ONE node-type descriptor (`src/app/overrideDescriptor.ts`) + ONE shared `OverrideDecorator` on the existing `ParamRow` now drive the per-field state-dot + ✕-revert for BOTH `MaterialOverride.overridden` AND `GltfChild.overridden` — the two surfaces that previously both rendered as the identical `(complex — Pro mode)` fallback. No per-consumer UI was written; the descriptor gates the decorator to the covered fields and returns null for the ~38 other node types (the over-reach risk did NOT materialise). **The schema-shape split surfaced a real seam:** revert can't be one primitive — `clearOverride` (drop the key) is correct for MaterialOverride's `.partial()` set but would FAIL GltfChild's fixed-key `z.object({position,rotation,scale})` validation, so `buildRevertedSet` branches `clearOverride` (sparse) vs `withOverride(false)` (record). Both still read not-overridden via `isOverridden`, and the renderer restores source because both consumers branch on the explicit bit (the same R-4/[[V28]] property the revert relies on). **Latent gap closed:** NPanel TRS edits on a GltfChild previously did NOT set the `overridden` bit (only the gizmo's `writeGltfChildOverride` did — the [[H40]]/C2 second-surface trap); the shared `dispatchOverrideValueEdit` now marks value + bit in one atomic on BOTH the gizmo and the Inspector. The MaterialOverride decorator is narrowed to `roughness`/`metalness` (the bit-consulting fields) — color/opacity/emissive are always-applied tints, so a decorator there would imply a non-existent inherit-vs-override choice. GOAL observed live (H40/H59, not node params): `tests/e2e/p130-npanel-override-decorator.spec.ts` edits metalness in the REAL NPanel → forced over the map (`.metalness===0`, dot fills), ✕-revert → map defends (`.metalness===1`, dot hollow). **#124's deferred halves are now BOTH shipped (#130 + #131); the override-consolidation boundary is complete** — the next override domain inherits the primitive + the descriptor + the decorator for free. (Self-review follow-up **#136 SHIPPED, PR #137 MERGED `f9e86d8`, 2026-06-02**: the missing `BooleanField` ParamRow renderer landed — `MaterialOverride.ignoreSourceMaterial` (the #131 flatten toggle) + 5 pre-existing booleans (loop×2/mute/solo/smaa/randomYaw) are now UI-clickable, not setParam/agent-only. `accent-accent` control tint is not contrast-gate-scanned; UI-SPEC D-UX-20, observed live in `tests/e2e/p136-npanel-boolean-toggle.spec.ts`. NPanel ParamRow now covers all four param primitives: number / vec3 / string / boolean.)
...
## B19 — Boundary: Material authoring (OpenPBR IR ↔ THREE adapter) — v0.6 #2

ORIGIN: The v0.6 #2 design pass (2026-06-06). The discuss-phase surfaced that a primitive's material is uneditable (`NPanel.tsx:636 (complex — Pro mode)`) and that the fix is NOT a one-off widget but a new domain boundary: a renderer-agnostic OpenPBR material IR compiled to a THREE material through ONE adapter. Deep external grounding (Blender/Houdini/OpenPBR/MaterialX/three.js TSL, source-level) decided the architecture (see DCC-ref §11a, [[V32]]).

WHY: Without this boundary tracked, the next material edit gets diagnosed from scratch and four failure CLASSES stay invisible: (1) the **migration look-shift** — the widen is a v1→v2 schema migration; a saved box must render byte-identically (new lobes off at weight 0), so MIGRATED nodes seed CURRENT-look constants (roughness 0.5, the renderer constant at `SceneFromDAG.tsx:666`) while NEW nodes seed the OpenPBR table (0.3) — two deliberate defaults; conflating them silently changes every saved project's appearance. (2) **type-guard narrowing after a widen** — `isMaterialSpec` (`resolveEvaluatedMesh.ts:54-61`) narrows on a top-level `color`; the widened IR moves it to `base.color`, so the unchanged guard returns false → material drops to `null` → mesh renders unlit with NO error (the [[V10]]/[[H14]] silent-stale-read class, must accept legacy OR widened). (3) **adapter drift** — inlining the IR→THREE mapping in N renderers re-introduces the [[V29]] N×M leak; it must be the ONE `openpbrToThree` site (BakedMeshR's MeshPhysicalMaterial build is the reference). (4) **two write paths under one surface** — a primitive OWNS its material (`setParam`) but a glTF child does NOT (per-`(child, slotIndex)` MaterialOverride, [[V28]]/[[H59]] map-aware); the uniform Inspector must route by target kind, and per-submesh `slotIndex` must stay backward-compat with whole-child #99/#124 (absent slotIndex = today).

HOW: Tracks the boundary spanning the new `openpbrToThree` adapter (`src/app/material/`, the DEEP module), the widened `InlineMaterialSpec` evaluator+consumer span ([[V10]]/[[H14]]), the leaf renderer consolidation (one MeshPhysical builder, [[V20]]/[[H36]]), the NPanel lobe editor (chrome → §B11 inventory + W8 a11y), the #149 animation reuse (channel `.sample()`, [[H40]] boundary-pair, Blender colour table), and the override-slot extension ([[V28]]/[[H59]]). **Observation targets (every behavioral e2e FALSIFIABLE, [[H40]]):** the real `mesh.material.<field>` == the resolver output (side-A/B equality); migrated-box evaluated roughness == 0.5 while fresh == 0.3; `isMaterialSpec` accepts both shapes; an edited glTF slot leaves sibling slots + textured-slot maps intact.

**STATUS (2026-06-07, branch `feat/v06.2-material-editing`, W0–W7 SHIPPED + falsified → PR #180, closes #178, NOT merged):** R1 (migration look-shift) OBSERVED + gated — `migrations.test.ts` proves migrated box = 0.5 / fresh = 0.3, the two-defaults split cannot silently converge (kept as the boundary's anticipation, not promoted; it was a designed default-split, not a bug). CAVEAT-1 (guard narrowing) OBSERVED + handled — `isMaterialSpec` rewritten dual-accept (`base.color` OR legacy `color`) BEFORE it bit; gated by a unit assert + the migration render path. **Two patterns promoted in W0-W5:** [[H73]] (`git checkout/restore <file>` to undo a falsification NUKES uncommitted wave work → commit-before-falsify) and [[H74]] (a paramPath rename's span includes the AGENT layer + LLM prompt + e2e corpus + sibling renderers, far beyond the V10/H14 schema-consumer list). **W6 (per-submesh glTF) SHIPPED + falsified:** the second write path is an OPTIONAL `slotIndex?: number` addressing dimension on `MaterialValue`/`MaterialOverride` (absent ⇒ whole-child = today, NO migration), un-collapsed `GltfAssetR` override effect (per-mesh `slotIdx` counter; `applies = override && (targetSlot===undefined || targetSlot===slotIdx)`; non-matching slots keep imported material; range-safe), NPanel `SlotSelector` enumerating slots via the production-safe `gltfCloneRegistry` (`src/app/resolveOverrideSlots.ts`). D-05 held — slotIndex is an addressing dimension, NOT a second code path; the glTF path reuses the EXISTING flat MaterialOverride controls (no lobe-vs-flat impedance), the router is the SectionCard conditional. Falsified (commit-first per H73): apply-to-all → slot-0-unchanged goes RED. p7.13/p124/p131 whole-child backward-compat stayed green; W8 a11y 8/8; vitest 1386; 21 e2e green. **One self-review pattern promoted:** [[H75]] (a conditional-render chrome that hides when its precondition lapses can strand a persisted optional value with no reset affordance → render-when-value-present). Self-review follow-up #179 (one MaterialOverride node = one slotIndex → can't author slot0≠slot1 from one node; renderer supports it, authoring-layer gap). **No hetvabhasa from the W6 execution itself — clean single-pass (no >1-attempt fix).**
...
**NEXT CONCRETE EXERCISE (Spline-exact chrome REDESIGN — branch `feat/v06.4-director-ux`, 2026-06-09):** v0.6 #4 Director-UX is feature-complete (W1-W5; W5 `cc73533`). The user then said the editor "doesn't look anything like Spline 3D — I want it exactly like this end to end" (showed the Spline editor: dark, ambient-glow, rich left outliner + right inspector, clean sans). Observed the gap live (screenshot): shipped chrome was LIGHT + monospace + dual cramped panels = a dev tool. **Scope LOCKED with user via AskUserQuestion: (1) DARK Spline-true (reverses the W3 light call — see [[V36]] bidirectional inversion); (2) KEEP the agent first-class, Spline-styled (NOT literal-Spline-no-agent — §196/D-03/[[V34]] thesis holds); (3) editor SHELL + home only (NOT the hero inflated-glossy material).** Plan = 6 waves: **A foundation (DONE `38c925c`) → B left scene outliner (DONE `d0185fd`) → C right inspector + re-home agent (DONE `30866fd`) → D top toolbar + bottom orbit-gizmo/ortho-persp → E home launcher → F polish + a11y re-baseline + screenshot-diff.** Wave A flipped tokens light→dark (accent → Spline blue `#5c9dff`), monospace→system-sans (repointed `mono` token; 17 `font-mono` classes flip globally — legacy name, later cleanup), index.css ambient corner-glow, dark viewport `#1a1b20`; re-ground the [[V36]]/[[H27]] gates in-wave (#57 worst-case DARK→BRIGHT, F7 literal, p57 re-grounded → surfaced [[H83]]).

**Wave B OBSERVED (`d0185fd`):** LeftSidebar rebuilt as the always-on Spline outliner (header + search + "Scenes" + per-type icon tree [NEW `SceneTreeIcon.tsx`] + Library/Import/Help footer); the W3 SCENE|AGENT tab strip dissolved. SceneTree rows restyled Spline (icon+name, hover fill, selected blue-tint + inset accent ring, `data-selected`; raw nodeId column dropped) + NEW `filter` prop. Boot default `leftSidebarCollapsed` true→false ([[H82]] inversion; collapse preserved per [[V35]]). §196 held because AgentChat ALSO mounts in the always-present RightDrawer (the left "agent" tab was a DUPLICATE — now single home). Footer reuse, no 2nd path ([[V34]]): Library→same AssetsPopover; Import→NEW shared `asset/importPicker.ts` (`openImportPicker`) extracted from MenuBar (−100 lines dup). spline-wb 4/4 FALSIFIED. D-01 (left default tab=scene) SUPERSEDED; D-03 (chevron) ADVANCED tab-strip→header.

**Wave C OBSERVED (`30866fd`):** user AskUserQuestion LOCKED agent re-home = **always-on bottom dock** (over summonable-float / right-tabs). NEW `AgentDock.tsx` (full-width 190px, above timeline) hosts the SAME unchanged AgentChat (§196 one component — relocating a mount is free because state lives in its zustand store). `RightDrawer.tsx` DELETED. Layout grid 4-col→3-col (dropped 280px `drawer`) + NEW `agentdock` row; inspector 280→300px + FULL-height; NPanel shell bg-muted/40→bg-2 + clean "Inspector" header. Inspector internals already collapsible-sectioned — NOT rebuilt ([[V34]]: didn't fabricate Spline scene-settings sections with no backing data). spline-wc 4/4 FALSIFIED (bounding-box geometry). D-UX-8 HELD (same NPanel restyled). Both waves hit [[H13]] (layout reframed viewport → postfx darwin baseline regen — done in-wave).

**NEXT WRITE = Wave D (top toolbar + bottom orbit gizmo + ortho/persp):** refine FloatingViewportToolbar to Spline's exact look (single rounded pill, monochrome icons, zoom %, Export, Play) + the centered axis-ball gizmo gains an Orthographic/Perspective segmented toggle. CARRY into Wave F: the AgentChat MODE row (read-only|copilot|sandbox) stretches across the full-width dock — needs proportionate content width. None merged; #4 stacks on #3; main still `a5fd4df`. DEV seam `__basher_three` (threeRef) from W5. Per-wave gate UNCHANGED (tsc -b/eslint0/format/vitest/falsified-e2e/W8+contrast on chrome DOM/section-inventory H25/H27); [[V36]] inversion re-grounding is ROUTINE on any token-polarity change; [[H13]] postfx-baseline regen is ROUTINE on any viewport-resizing chrome change.

**Execution observation targets (run at the Write to each seam — these are the leak-check + the per-wave traps the plan PINS):**

- **W2 mode removal — OBSERVED RESULT (PASSED the leak-check, 2026-06-08, commit `b5af221`):** the substrate-purity seam held — `presentMode` (chromeStore), the Play transport (useTimeStore), and the timeline reveal (viewportStore) are all EPHEMERAL UI-projection state, non-persisted, V34-clean (no DAG dispatch passes through them — the mode was never in the DAG). NO bolted-on "advanced" surface was introduced; the timeline is the SAME projected graph revealed via progressive disclosure (leak-check #3 satisfied). The deletion regression **grep gate** is LIVE in `grepGates.test.ts` (keys on `useModeStore`/`stores/modeStore`, excludes the two homonym `setMode` vocabularies — `useAgentSessionStore` autonomy + `gizmoStore` tool dispatch — by token, not bare `setMode`); the `FloatingViewportToolbar.tsx` header comment token was scrubbed (MUST-FIX #1). Two execution learnings catalogued: [[H79]] (the test-corpus blast radius was 18 specs not the ~6 planned, AND a blanket find-replace double-toggled the multi-behavior specs — `animate` bundled slot-reveal + keyframe-enable, which the new design splits) and [[V35]] (the timeline reveal must stay reachable while collapsed → slot ALWAYS mounted, body gated — MUST-FIX #3; cost = ~39px viewport, W1 can move the reveal to the pill and reclaim it).
- **W1 consolidation — OBSERVED RESULT (PASSED the leak-check, 2026-06-09, commit `bda0cac`):** ToolRail/Chrome/TopToolbar DELETED; the four tools + Add/Assets/space/zoom/Export/Present/Play/timeline-reveal collapsed into ONE floating pill (`FloatingViewportToolbar`, mounted at `<main>` so the Space toggle works in UV); Chrome's brand/save/ProjectsMenu folded onto the ProjectTabs identity bar. Substrate-purity held — no authoritative state moved, only DOM homes; every control has exactly ONE home (falsifiable `p6-w1-consolidation.spec.ts` W1#2 count===1, falsified by re-mounting a 2nd pill → count===2). V19 single-writer preserved. **The carry-over [[V35]] "reclaim the ~39px by collapsing the timeline slot" was ATTEMPTED then REVERTED** — observation (5 failing `p7-animation-authoring` autokey specs) showed the always-visible Timebar HOSTS the Auto-Key record indicator (footgun mitigation, must be unmissable); collapsing it re-opened the silent-data-loss footgun → kept the bar, kept the pill reveal. New catalogue [[H80]] (space-reclaim collapse must not hide a load-bearing safety indicator the bar hosts — Chesterton; relocate-before-collapse, never silence-the-test). H79 blast-radius recurred as predicted: the timeline-reveal migration touched ~15 e2e specs (`timeline-drawer-toggle`→`floating-toolbar-timeline`, all `.click()` → safe blanket sub since both are toggles) + contrast matrix re-homed (R4 pruned, R3 buttons keep their own bg so rows stay valid on the pill, Chrome rows → ProjectTabs bg-2/80). NAMING DEBT (self-review): the moved chrome controls KEPT their `top-toolbar-*` testids on the bottom pill (preserved to avoid churning ~8 spec files) — cosmetically misleading, flagged as a follow-up.
- **W3 calm LIGHT palette — OBSERVED RESULT (PASSED, 2026-06-09, commit `29a3afe`):** full dark→light token inversion in `tailwind.config.ts` (neutral ramp + canvas tint via named `viewportColors.ts`); semantic hues darkened to stay AA. Gray-area decision surfaced to the user (the neon accent can't serve as text/border on light): grounding flipped the cost — "neon as fills/borders" was ~80 sites (the accent is the universal focus-ring color) AND infeasible (neon border < 3:1 on light), so the user chose **darken the single `accent` token, zero call-sites** (the `text-bg` knockouts auto-invert because `bg` went light). New invariant [[V36]] (a semantic hue must clear AA-text + SC1.4.11-border on every surface polarity; darken the token, don't split roles; + the corollary [[H27]] inversion re-grounding). Re-grounded the over-canvas worst case white→black (the matrix `#57` `it()` + `p57` e2e) — the dark-backdrop flip immediately caught a real 4.48:1 the white version masked. Falsified post-commit (neon accent → 13 rows + focus-ring gate fail); OBSERVED the render (calm light, legible). F2 TOKEN-drift + F7 PAGE_BG gates updated in-wave.
- **W4 Spline HOME + first-run routing — OBSERVED RESULT (PASSED, 2026-06-09, commit `ff374ec`):** `examples.ts` (Op-built-DAG examples, V34-proven via `examples.test.ts`), idempotent boot seeding, `routeStore` (ephemeral, V34-clean), `Home.tsx` launcher (reads `listAllProjectMetadata`, delegates CRUD to boot helpers — no state outside the IR), App home-XOR-editor branch (single R3F canvas, never double-mounted), back-to-home ⌂ on the W1 identity bar (no new band). The `:137` persist-on-boot trap was guarded (first-run does NOT persist/hydrate) + stale-id clears+home + canonical-`default` rebuilds (never strands). Two execution traps catalogued: [[H81]] (the first-run `return` sat ABOVE boot's DEV-seam block → would strand `__basher_*` on the home route → rewrote to if/else fall-through) and [[H82]] (first-run→home inverts the implicit "`/`=editor" contract the WHOLE e2e harness relied on → re-established at the `_fixtures.ts` chokepoint with a CONDITIONAL `lastProjectId='default'` seed, only-when-absent so reload-resume survives; per-test override for the home spec). Proven: p6-w4-home 6/6 + p26 12/12 + p6-w8 8/8 still resume the editor; OBSERVED the home render (calm gallery + examples). Per-card rename/dup-from-home deferred (boot wrappers current-scoped); live thumbnails deferred (D-W4-THUMB).
- **W2↔W3 coupling** ([[H27]] drift): the contrast matrix rows (`contrastMatrix.test.ts:328-358,736-741`) + 5 e2e specs asserting `data-mode` (p6-w2/w7/w8, acceptance, `p57-bright-scene-contrast.spec.ts:151,201,225` + `SceneBgTestSeam.tsx`) reference surfaces W2 deletes → MUST be updated in the SAME wave or the gate goes stale.
- **W4 home / boot routing** ([[H78]] sibling — a one-shot signal defeated by an unconditional writer): `boot.ts:137` persists `lastProjectId` UNCONDITIONALLY → "first-run = absence of lastId" is true for exactly ONE boot unless the first-run path is guarded to NOT persist/hydrate. Examples = Op-built DAGs (V34-clean, NOT static JSON); `routeStore` ephemeral; App branches home XOR editor AFTER boot-ready so the single R3F Canvas (`Viewport.tsx:105`, V8/V11) never double-mounts. Reuse existing project CRUD (`boot.ts:561-653`) — do NOT reimplement.
- Every chrome wave ALSO runs the §B11 SECTION-INVENTORY re-validation ([[H25]]/[[H27]]), the [[H75]] conditional-render-strands-a-value check, and W8 a11y on any chrome DOM change. Audit OUTPUT `docs/UI-REVIEW.md` (W1 = CONSOLIDATE). Plan checks: `PLAN-CHECK.md` (W1/W2/W3) + `PLAN-CHECK-W4W5.md` (W4/W5), both PASS-WITH-CAVEATS, must-fixes applied.

### Boundary B20: Dark-redesign chrome surface ↔ the shared ambient stage + token-audit gate (the Spline A–F redesign boundary)

**ORIGIN:** Spline Wave E home restyle, 2026-06-10 — the `Home` launcher had been left in the pre-redesign dev-tool look (opaque `bg-bg` that OCCLUDED the body ambient glow, `font-mono`, uppercase dev-chip headers). Restyling it surfaced (a) the glow-occlusion bug ([[H85]]) and (b) the contrast-matrix coverage gate firing on every new translucent/gradient token + a token-shaped string in a COMMENT (a recurrence of the B18 "gate greps comments" note, now at a non-toast surface → generalized). The redesign is ONGOING (waves remaining + future full-page routes), so the next chrome wave inherits these checks.
**WHY:** Without this boundary tracked, each new dark-redesign surface re-derives the same two mistakes from scratch: it paints an opaque page bg (disconnects from the lit stage, then gets a drifting parallel glow), and it invents new token variants (trips the contrast gate, tempts whitelist-bloat). Both look "fine" in isolation — the surface renders — so neither is caught until a screenshot-diff or the unit gate fails. The cost is a re-debug per surface instead of a matched check.
**HOW:** ONE rule with two faces (V39): full-viewport surfaces render TRANSPARENT over the `body` ambient glow (defined once, index.css) — never an opaque page-level bg; panels on them stay translucent (`bg-2/90`/`bg/95`) so the glow reads through. AND reuse the editor's ALREADY-AUDITED token vocabulary so the `contrastMatrix.test.ts` coverage+border gates stay green without new ROWS. New tokens, when genuinely needed, get classified (ROW / WHITELIST / BORDER_DECORATIVE), not silently passed. Sans (Wave A repointed the `mono` token to sans globally — `font-mono` is INERT, so a "not monospace" test does NOT bite a re-added font-mono; assert the real Wave-E/D signal instead, e.g. headers normal-case via computed `textTransform`).
**REF:** [[H85]] (glow occlusion), [[V39]] (transparent-over-stage + token-reuse invariant), [[V34]] (one treatment reused), [[B18]] (the original "gate greps comments" note, toast-scoped), `src/index.css` (body glow), `src/app/Home.tsx` + `src/app/Layout.tsx` (surface roots), `src/a11y/contrastMatrix.test.ts` (the token gate), `tailwind.config.ts` (mono→sans repoint), `tests/e2e/spline-we-home.spec.ts` + `spline-wd-toolbar.spec.ts` (falsified visual contracts). Spline reference: `docs/SPLINE-UI-REFERENCE.md`.
**Silent-failure modes:** opaque page-level bg on a route root → occluded glow, looks like a separate product, "fixed" by a parallel drifting glow ([[H85]]); a new translucent/gradient token (or a token-shaped substring in a COMMENT — `bg-…/…`, `border-<word>`) → contrast-gate coverage/border FAIL that reads as a real a11y problem; re-adding `font-mono` does NOTHING (mono==sans now) so a font-based test silently passes regardless — false confidence.
**Observation targets:** screenshot each new dark surface and look for the corner blooms (absent ⇒ occluded); computed `backgroundColor` of a full-page root should be `rgba(0,0,0,0)`; run `contrastMatrix.test.ts` after ANY chrome edit and reuse-or-classify what it flags; for a visual falsifier, assert the actual redesign signal (headers normal-case, glow transparent, cards elevated) — NOT the font, which is inert. §196 every wave: the agent dock + Add/Assets survive.

**Wave-F extension (2026-06-11) — the INERT-FALSIFIER discipline (generalized from the font-mono note above).** A falsifier must bite the REAL mechanism on THIS platform/layout, not a proxy that is structurally inert. Three inert proxies hit during Wave-F polish, all green-when-they-should-be-red until caught by running them red: (1) `font-mono` "not monospace" — inert (mono==sans); (2) clamping a scroll container's height to "force overflow" — tests CSS, not whether the layout BOUNDS the panel ([[H86]]); (3) the scrollbar-GUTTER check (`offsetWidth-clientWidth`) for a hidden bar — inert on macOS/headless overlay scrollbars (no gutter either way) → use computed `scrollbar-width`. **Rule:** before trusting a falsifiable test, REVERT the fix and watch it go red here; if it stays green, the assertion is inert — find the signal that actually moves (inject content, computed style, real node-count). This is the same Lokayata gate applied to tests: observe the falsifier biting, don't infer that it would.
**Wave-F extension — editor chrome over the viewport must be MASKED in the postfx-beauty screenshot ([[V37]] into the test domain).** Moving the floating toolbar to `top-4` put it prominently inside the `viewport` screenshot region, breaking the pure-DAG `postfx-beauty.png` baseline. The beauty test already masks `fps-meter` + `viewport-empty-hint` ("immune to chrome copy/size"); the toolbar is chrome too → added it to the mask, then regen. Rule: any editor chrome composited over the `viewport` testid element (toolbar, HUD, hints, gizmo overlays) is masked in `acceptance.spec.ts #7`, so its position/contents never tip the DAG-render diff. REF: `tests/e2e/acceptance.spec.ts` (#7 mask list), `src/app/FloatingViewportToolbar.tsx` (`top-4`), [[V37]]. Commit `a155370`.
**Wave-F extension — the agent dock is content-sized, not a fixed row.** The bottom agent dock row was `190px` fixed and filled an empty `flex-1` message list → a tall void above the input. Fix: grid row `auto` + the message list mounts ONLY when there's a conversation (capped `max-h`), so an empty chat collapses to the input bar; the viewport reclaims the height. Same `min-height`/content-sizing family as [[H86]]. REF: `src/app/{Layout,AgentDock,AgentChat}.tsx`, commit `9701498`; the bare single-bar + `>` icon send is `d74a9ce`.

### Boundary B21: Native `.basher` scene file ↔ the global OPFS asset stores (the portable-scene boundary)

**ORIGIN:** `.basher` single-file scene format, 2026-06-11 — the user wanted to save/share a whole scene as ONE self-contained file. Export already emitted a DAG-only `.basher.json`; there was no importer and no asset embedding. Building it surfaced the boundary between the DAG's asset HANDLES (assetRef strings, baked geometry/texture refs) and the GLOBAL, content-addressed OPFS stores those handles point into (`user-imports/`, `baked-geometry/`, `baked-texture/`) — stores SHARED across every project, not per-project.
**WHY:** Without this boundary tracked, the next asset-bearing feature re-derives two mistakes: (a) it embeds too little — a per-node-type collector that misses a new handle shape ships a "portable" file that silently drops an asset and breaks on another machine ([[H77]] substrate leak, invisible until cross-machine open); (b) it embeds too much — embedding a whole shared store leaks OTHER projects' assets into the file. Both render fine in isolation; neither is caught until a cross-machine open or a privacy review. The cost is a re-debug per asset feature instead of a matched check.
**HOW:** ONE shape-driven collector ([[V41]]) walks the DAG params for the THREE handle shapes (user-imports/ string → whole folder; baked GeometryDescriptor → bin path; BakedTextureRef → baked-texture by hash) wherever they nest — so PRECISELY-referenced assets are embedded, and a new handle-bearing node is captured automatically. App-shipped `assets/` are EXCLUDED (re-seeded everywhere). Import = rehydrate write-if-absent (content-addressed baked-\* are byte-identical on collision; user-imports left intact on same-name) → new-id Project through loadProject's exact ladder (migrate→ProjectSchema.parse→migrateNodes) → createNewProject hydrate (non-destructive). The envelope is backward-compatible with the legacy DAG-only `.basher.json` (assets optional).
**REF:** [[V41]] (the complete-span + shape-driven invariant), [[H77]] (the substrate leak this guards), [[H87]] (the same-scene round-trip inert falsifier), [[V34]] (one substrate), `src/app/sceneBundle.ts` (collector + base64 + load ladder), `src/app/boot.ts` (`buildSceneBundleForCurrent`/`importSceneBundle` + `__basher_export_scene_bundle`/`__basher_import_scene_bundle`/`__basher_opfs` seams), `src/app/sceneFileActions.ts` + `src/app/MenuBar.tsx` (Save/Open), `src/app/exportDag.ts` (legacy DAG-only sibling), `tests/e2e/menu-scene-file.spec.ts`. Commits `44d1025` (core) + `090370f` (orchestrators+UI) + `1ecda1a` (e2e). Branch `ux-overhall`. No issue (user idea, live build).
**Silent-failure modes:** a new OPFS-backed handle shape not matched by the collector → silently dropped from the bundle → blank/errored node on cross-machine open ([[H77]]); embedding a whole shared store → other projects' assets leak into the file; a same-name-different-content `user-imports` folder on the importing machine → write-if-absent keeps the existing (possibly wrong) asset (known v1 limitation, non-destructive by choice); a same-scene round-trip test that can't tell import-worked from import-no-op ([[H87]]).
**Observation targets:** for any new asset/handle feature, ask "does `collectAssetRefs` capture this shape?" — add a unit case + an end-to-end delete-then-rehydrate e2e; for any round-trip test, diverge the live scene before opening; confirm the falsifier bites red by reverting the mechanism. The export's `missingAssets` list must be surfaced (V38), never a silent partial file.

### B1.1 + B14 v0.7 extension 2026-06-17 (#195/#197 — the DIRECT-CHANNEL overlay is a NEW BAND on the existing resolver boundary, no new boundary; AnimationLayer is being absorbed, not replaced)

**ORIGIN:** the v0.7 unification (epic #195) — the user reframed #188 (glTF material animation) as the real problem: "animate a node's params over time" runs through THREE mechanisms (AnimationLayer wrapper / glTF baked channels / camera channels), and AnimationLayer is the lone legacy holdout. Phase 2 (#197) gave native meshes the camera/glTF direct-channel road.

**WHY:** without this entry the next animation surface (glTF materials #188, lights, any param) would be added against whichever of the three mechanisms the author stumbled into, re-threading two callers each time — the span-mismatch the unification resolves ([[V57]]). And the coexistence double-overlay ([[H105]]) is invisible until a layer-animated node renders doubled — a silent failure the boundary's observation target must name.

**HOW:** the direct-channel overlay (`overlayChannels`, [[V57]]) is the SAME "one band, two callers" structure B1.1/B14 already enforce — applied at BOTH the render side (`DirectChannelsR`, SceneFromDAG, useFrame snapshot) AND the read side (`resolveEvaluatedTransform`, `ctx.time`), channels→transient order. It is a NEW BAND (free-floating channels by `target`), NOT a new boundary. The ONE overlay primitive is `patchTarget` lifted out of `AnimationLayer` (so the wrapper now delegates to it) — the wrapper is being absorbed (Phase 5), not replaced by a parallel path.

**Silent-failure modes (NEW):** (a) a by-target channel enumerator that does not exclude layer-wired channels → double-overlay during coexistence ([[H105]]) — gated by the `nodeChannels.ts` coexistence guard + unit test; (b) render overlays but read-side does not (or vice-versa) → gizmo/inspector freeze while viewport animates ([[H40]]) — gated by the p197 boundary-pair (Side A == Side B); (c) the per-node render subscription not narrowed (shallow on channel node refs, mounted only for `directChannelTargetSet` members) → an [[H48]]/B13 re-render storm; (d) authoring still mints layers (Phase 2 left the switch to Phase 5) → new native animation uses the layer path, not the road — intentional during coexistence, NOT a bug.

**Observation targets:** for any NEW animatable surface, ask "does it use the direct-channel road ([[V57]]) — channels by `target`, overlaid by `overlayChannels`, on BOTH render + read?" and "does the by-target enumerator subtract the layer path's claim ([[H105]])?". Drive the boundary-pair by INJECTING a free-floating channel via `__basher_dag` and asserting the EVALUATED render == the resolver (the V56/p197 method). REF: [[V57]], [[H105]], [[V56]], [[H40]], [[H48]], `docs/UNIFICATION-DESIGN.md`, `src/nodes/overlayChannels.ts`, `src/app/nodeChannels.ts`, `src/viewport/SceneFromDAG.tsx` (`DirectChannelsR`), `src/app/resolveEvaluatedTransform.ts`. Commits `6b47f2a`/`ed9b418`/`7cd9f1d`/`7ebea2a`. Issues #195/#196/#197 (Phase 3 #188, Phase 5 #199).

### Boundary B22: Operator substrate (constraints/modifiers as typed DAG chains) + the world-transform-purity gate — EPIC #201 COMPLETE (#202 gate + #204 CHOP/constraints [[V60]] + #205–#208 lighting [[V61]]/[[V62]]/[[V63]] + #209 SOP/modifiers [[V64]]; BOTH operator halves of [[V58]] now IMPLEMENTED)
**ORIGIN:** 2026-06-18 design discussion. A user request for a Blender-Light-Studio-style lighting workflow, pulled to its root, surfaced a foundational decision: constraints and modifiers are ONE pattern ([[V58]]). Filed as epic #201 (slices #202–#209), design doc `docs/OPERATORS-AND-LIGHTING-DESIGN.md`.
**WHY:** Without this boundary tracked, two traps recur. (1) The next "relationship" need (camera-tracks-subject, light-aims-at-rig, prop-in-hand) gets built as another one-off special case — the camera's intrinsic `lookAt` ([[V56]]) is ALREADY the first special case, and a second one-off deepens the inconsistency the user explicitly flagged. (2) The world-transform-purity gap is invisible until a constraint needs it: a constraint must read its target's WORLD transform, but world transform is composed DOWNSTREAM in `SceneFromDAG`, not as a pure evaluable value — so a naive constraint reads the scene-composition (impure → [[V37]] render-parity break) instead of a pure resolver. This is the exact [[B1.1]]/[[H40]] class ("which side of the producer/consumer boundary did I observe?") at the world-transform level.
**HOW:** the substrate is the EXISTING DAG — operators are `NodeDefinition`s, the constraint target is a DAG input edge (so the evaluator's topo-sort + cycle detection order it for free), and the shared `OperatorStack` is the ONE new abstraction (transform stack + geometry stack). The foundational slice #202 lifts world-transform composition into a pure `resolveWorldTransform(state, nodeId, ctx)` that mirrors (Chesterton — reuse, don't re-derive) the `SceneFromDAG` accumulation. Slice #204 migrates the camera `lookAt` onto a Track-To constraint (the dogfood proof). Studio lights (#205) aim via that same Track-To. **REF:** [[V58]] (the invariant), [[V56]] (camera aim — migration target), [[V37]] (render parity — every operator pure), `docs/OPERATORS-AND-LIGHTING-DESIGN.md` (§4.3 the gate, §9 sequencing), `src/viewport/SceneFromDAG.tsx` (where world transform is composed today — the gate site), `src/core/dag/evaluator.ts` (topo-sort + cycle detection — free ordering), epic #201.
**Silent-failure modes:** a constraint reads `SceneFromDAG`-composed world transform → render ≠ viewport, and it can't run in pure eval; a constraint stores a baked rotation → desyncs when subject/light moves; a relationship built as a one-off (not on `OperatorStack`) → the chain wiring/UI/serialize/mute duplicated; a parent class designed above SOP/CHOP/VOP → ceremony over `NodeDefinition`; the full Blender constraint list built before the stack + world-transform model are proven.
**Observation targets:** slice #202 — **DONE (`48f1c17`/`484740c`, ux-overhall).** `src/app/resolveWorldTransform.ts` lifts the world-transform composition into a pure resolver mirroring `SceneFromDAG` ([[V59]]). The boundary-pair gate PASSED (observed, not inferred): pure `resolveWorldTransform` (side B, `__basher_world_transform`) == the rendered world matrix (`__basher_mesh_world_position` seam) for a nested + animated Transform→Box hierarchy at 2 playhead times — sideA==sideB [1,0,0]@t0, [11,0,0]@t2 (`tests/e2e/p202-world-transform-boundary-pair.spec.ts`); falsify by reverting the pure lift → values diverge from render. 9 unit tests lock the composition math (`src/app/resolveWorldTransform.test.ts`). Slice #204 — **mesh Track-To DONE (`366f11d`/`2d2edc6`/`cab0e1f`, ux-overhall).** The general constraint shipped FIRST (user's chosen order over #203): `resolveTrackTo` (pure aim) + `TrackTo` node + `nodeConstraints.ts` (scene-layer resolve, V59's first consumer) + `ConstrainedR` (render) + `resolveEvaluatedTransform` (read) → [[V60]]. Boundary-pair PASSED (observed): a box's rendered -Z tracks a MOVING target == the read resolver == the physical dir, +X@t0/+Z@t2 (`p204-track-to-boundary-pair.spec.ts`, `__basher_mesh_world_quaternion` seam); a node-ref constraint follows the moving target; A↔B mutual tracks both terminate (no cycle — resolveWorldTransform is constraint-free). **KEY REFINEMENT learned (update the design's §2.2 lean): a RELATIONSHIP constraint canNOT be a wrapper sub-chain node — the aim needs world position a bare evaluate lacks — so it resolves at the scene layer, edge-less + enumerated like a channel (the resolveActiveCameraPoseAt/V56 pattern); the sub-chain model still fits SOP/geometry operators.** **Camera migration DONE (`fe62ca6`/`033e6d3`) — #204 COMPLETE.** The camera's bespoke aim is deleted: `resolveActiveCameraPoseAt` derives `lookAt` from a Track-To via the SAME `resolveTrackToTarget` meshes use (the camera expresses the aim as a lookAt point; Object3D.lookAt == resolveTrackTo's Matrix4.lookAt). Observed (`p204-camera-track-to.spec.ts`, REAL look-through camera forward tracks a moving target == constraint); no-constraint path byte-identical (returns null pre-eval); per-frame look-through resolve cached (no re-walk); p190 regression-green. NOTE: world-transform of a glTF child / Scatter instance / Character bone is OUT OF SCOPE in `resolveWorldTransform` + V60 today (descent stops; returns null) — extend there if a constraint must track those; nested constrained nodes (parentWorld⁻¹·aimWorld) also a follow-up. #203 `OperatorStack` DEFERRED until a 2nd operator consumer exists (Vairagya — a chain/stack abstraction with one operator is premature). **Slice #205 StudioLight + LightRig DONE (`a7b47e2`/`8f7d22b`, ux-overhall) — the first lighting consumer of Track-To.** The user reframed it (adopted, Reception): NOT a new `StudioLight` node — the EXISTING `AreaLight` + an optional `tex` ([[V61]]; V34/V58 — one substrate, no parallel system). Two increments: (A) the §1.5 render PAIR — `StudioAreaLightR` expands a textured AreaLight into a mean-radiance-tinted `RectAreaLight` (`averageRadiance`) + an emissive textured card; unset tex → byte-identical plain light. (B) the LightRig — area lights aim via Track-To (`useAreaLightAim`, the EditorViewCamera per-frame pattern), the THIRD V60 consumer (mesh · camera · light): the rig is a Track-To consumer, not a bespoke mechanism. Boundary-pairs observed (`p205-studio-light.spec.ts`, REAL render): the RectAreaLight's color chroma == the card texture's mean-radiance chroma (render==resolver); a Track-To aims the light's world -Z at `normalize(aimPoint−position)`, overriding the authored lookAt; plain light → no card (falsification); offscreen render succeeds (V37). New trap [[H109]] (raw half-float bits ≠ decoded — decode both sides of an HDR boundary-pair). KNOWN-LIMITS (v1, §1.5): no area-light shadows (path-tracer later), no inspector `tex` picker yet (the #206 2D panel is the authoring home), emission params (opacity/falloff/saturation/mute) deferred to #206 (Vairagya). **#206 (2D panel) IN PROGRESS — increment 1 (the placement resolver) DONE (`aedf704`):** `src/app/resolveStudioLightTransform.ts` — `resolveStudioLightTransform(panelXY, radius, target) → {position}` (lat-long canvas point → world position on the rig sphere; the light aims back via its Track-To V60) + the inverse `studioLightPanelXY` (existing light → puck spot); the V51/V56 one-pure-resolver shape, 9 unit, round-trip exact. **#206 DOCKING (user via AskUserQuestion): a THIRD TAB in the timeline drawer** (Dopesheet|Curve|Light Studio), reusing `TimelineDrawer`+`timelineDockStore`; I flagged the semantic caveat (the drawer is a TIME surface) — user chose it for chrome reuse. **#206 scope (user): BLS 3D-lights panel ONLY — DEFER the HDRI-Light-Studio-Pro paint-into-global-HDRI mode (do NOT re-propose).** **#206 COMPLETE (`5f6e12a`→`8024f4b`, 2026-06-19) — the panel UI shipped, increments 2–4 → [[V62]].** (2) read-only canvas + pucks (`studioLightPanelXY` projection, a third timeline-drawer tab; `studioLightRig.ts` enumerate + `resolveRigTarget`; `studioPanelGeometry.ts` the v-flip in ONE place per H95); (3) drag-to-place (pointer-capture → `fractionToPanelXY` → `resolveStudioLightTransform` → `setParam` position, radius frozen at grab); (4) light list + per-light params + the `tex` PICKER (`StudioLightControls`, reusing `importEnvironmentHdri`) + "+ Light" (`buildAddStudioLightOps` — atomic AreaLight→scene.lights + TrackTo aiming the centre) — **CLOSES the #205 no-tex-picker known-limit.** Observed (`p206-light-studio-panel.spec.ts`, LIVE app): puck == `studioLightPanelXY` (panel==resolver); drag → DAG position == resolver == a live RectAreaLight (panel==viewport, V37); "+ Light" → puck+row+selected controls; tex picker → a live emissive card (the §1.5 pair); falsified by a free light (no puck) + plain light (no card). RIG-CENTRE derives from the lights' shared Track-To aim (default origin); explicit `LightRig` node deferred to #208. Two self-review fixes folded in: a FRESH EvaluatorCache per recompute (a memoized-once cache fed a stale aim-node world after the node moved, `cebdfea`); restored `onClick` for keyboard puck-selection (`1b19c6a`). #206 KNOWN-LIMIT (v1): no ◇ keyframe-diamond IN the panel ([[H104]] — custom controls bypass the generic diamond; params are keyable from the dopesheet/inspector, just not from this surface); the env-HDRI canvas backdrop is a later nicety. **Slice #207 the Light Brush DONE (`da8af37`→`f7d77f4`, 2026-06-19) — a SECOND authoring surface onto the same rig-sphere coordinate system ([[V62]] generalized).** Pure core `resolveLightBrushPlacement` (reflect(viewDir,normal) for a highlight, or the normal, ∩ the SAME rig sphere the panel drags on, radius preserved); decision layer `buildLightBrushOp` (pure over state+hit, only a selected rig light is brushable); `lightBrushStore` (transient, like the gizmo flag); the impure shell is a branch in `SceneFromDAG.onClick` (extract hit/world-normal/view-ray from the R3F event → setParam); panel toggle `LightBrushControls`; `Viewport.onPointerMissed` guarded so a missed stroke keeps the selected light. Observed (`p207-light-brush.spec.ts`, LIVE): brush-on click moves the light onto its radius-shell + a live RectAreaLight follows (panel==viewport, V37); brush-off click leaves it (falsification). New trap [[H110]] (an e2e viewport click can land on a floating DOM island / camera-helper over the canvas → never reaches the mesh onClick; `elementFromPoint` names the occluder; move the target mesh to clear screen space). #207 KNOWN-LIMIT: brush paints only `SceneChildNode` meshes (helper-click selects it); no in-viewport brush cursor; non-uniform normals approximate. **Slice #208 profiles — SUBSTRATE DONE (`a084422`/`ae2739f`, ux-overhall) → [[V63]].** The user chose (AskUserQuestion) the "LightRig node + LightProfileSelect" shape over edge link/unlink and over app-layer JSON snapshots (the latter rejected for a second source of truth, V34). Increment 1: `LightRig` node groups its lights + owns explicit `center`/`radius` (the explicit rig deferred from #206/#207 now landed); `Scene` gains a SEPARATE `lightRig` input (never merged into `lights` → direct-light index-correspondence byte-identical); `resolveRigLightSources` recovers rig-light node ids in edge order so Track-To aim + click-select survive the new render band. Increment 2: `LightProfileSelect` (the ClipSelect pattern) switches the live profile by one keyframeable `setParam`, all rigs co-resident (V34/V57). Observed (`p208-light-rig.spec.ts`, LIVE, 3/3): a rig renders its grouped lights + a Track-To still aims one (node-id recovery proven); a `setParam` flips Key(2)→Rim(1); an unwired rig renders nothing (falsification); offscreen parity holds. KNOWN-LIMITS (app-layer pending): no "+ Profile"/profile-switcher UI; existing studio lights not yet migrated into a default rig; the panel still enumerates ALL rigs' lights + `resolveRigTarget` still derives the centre (not the rig's explicit `center`); JSON import/export not yet built. **#208 app-layer + JSON I/O DONE (`c4a047c`/`ca08154`/`463907a`) — #208 COMPLETE.** Grounded in BLS `light_profiles.py`+`gui.py` (read at `/tmp/bls-study`): a **Profiles bar atop the Light Studio tab** (user picked via AskUserQuestion — HDRLS+BLS both keep setup mgmt WITH the canvas). `studioProfiles.ts` (reader + add/select/delete builders + `uniqueProfileName` collision guard; FIRST profile ADOPTS legacy free studio lights so scoping doesn't hide them); panel now profile-aware (`enumerateActiveProfileLights` + `resolveActiveRigCenter`). `studioProfileIO.ts` (portable `.bls`-shaped JSON: compose/parse/import; tex ref carried, bytes are the .basher bundle's job V41). Observed (LIVE, p208-profiles-ui + p208-profiles-io): add→2-light→empty 2nd→switch re-scopes 2↔1→delete re-points; Import rebuilds + names switcher; malformed→error. REMAINING KNOWN-LIMITS: no rename-in-place (rig.name+select.selectedProfile must update together); Export-all only; env-HDRI backdrop later; no ◇ keying in panel (H104). **Slice #209 SOP/modifiers — DONE (`2f0537e` operator + `f7e2bbf` OperatorStack + `9304876` stack UI + `fdd4886` agent op) → [[V64]]. The geometry half of [[V58]] is now IMPLEMENTED; epic #201 COMPLETE.** USER FORKS (AskUserQuestion, 2026-06-20): (1) FIRST modifier = **Array** (1→N copies merged — clearest parametric proof); (2) **LIFT #203 `OperatorStack` now** (the 2nd operator consumer earned the deferral). The #204 refinement was CONFIRMED here, completing the §2.2 picture: a geometry modifier needs only the mesh VALUE (never world position) → it IS the wrapper sub-chain node the doc imagined (`ArrayModifier`, `Mesh target → out`, edge-WIRED) — the EXACT contrast to the edge-less Track-To. So the sub-chain model that did NOT fit constraints DOES fit geometry. The lifted `OperatorStack` (`src/app/operatorStack.ts`) is therefore the GEOMETRY stack only — the doc's "polymorphic over transform+geometry" collapsed because constraints turned out edge-less. **Observation targets — ALL PASSED (LIVE, boundary-pair):** the modifier produces a REAL rebuildable `GeometryRef` the renderer (`ModifiedMeshR`) + read-side (`resolveEvaluatedMesh`) + offscreen render all consume identically (one shared `modifierGeometry.ts` projection → byte-identical key on both roads, [[H40]]/[[V37]]); `p209-array-modifier` — render verts (72) == resolver verts (`__basher_modified_vertex_count`), a 2-deep chain renders cumulatively (recursive `geometryRegistry.build('array')`); `p209-modifier-stack-ui` — the panel's "+ Array"/mute/delete drive the live render. The agent op (`mutator.geometry.addModifier`) goes through the SAME `buildAddModifierOps` (one wiring authority) and passes all five mutator gates (its contract honestly drops `'children'` from preserves — it re-routes the consumer's child edge — the V14 discriminator vs addChannel). NEW TRAP [[H111]] (clone-don't-mutate the cached registry source when building a derived geometry). KNOWN-LIMITS (clean future slices): only `ArrayModifier` v1 (Mirror/Subdivide register a type + a build branch); glTF/baked sources pass through (async geometry); `ModifiedMesh` UVs null; the gizmo on a selected modifier is inert (move the base, reachable nested in the outliner — `sceneTreeWalk` recurses the `target` input like Transform); the agent op is add-only.

Status: **EPIC #201 COMPLETE — #202 + #204 + #205 + #206 + #207 + #208 + #209 all SHIPPED. Both operator halves of [[V58]] IMPLEMENTED (CHOP/constraints [[V60]], SOP/modifiers [[V64]]); the `OperatorStack` §2.2 abstraction is lifted (geometry stack). This boundary is now a GUARD (the sub-chain-vs-scene-layer contrast is the deductive check for any future operator). NEXT operator work (out of this epic): more modifiers (Mirror/Subdivide), or the path-tracer that collapses the studio-light pair ([[v07_renderer_cpu-wall_webgpu]]).**

**POST-#209 — GUARD EXERCISED (the boundary held).** `MirrorModifier` (`582a327`, the 2nd geometry operator) was added by the deductive checklist the guard predicts — node + `geometryRegistry.build` branch + the `modifierGeometry` projection key + the `resolveEvaluatedMesh` read-side twin + 4 registrations — with ZERO change to the OperatorStack / `ModifiedMeshR` / the recursion machinery → [[V64]] generalization PROVEN. The one genuinely-new mechanism (a det−1 reflection flips winding) became its own trap [[H112]] (reverse the reflected copy's winding), NOT a structural change — the boundary absorbed it. A self-review finding, promoted to a tracked gap and then RESOLVED in-session (`4a39ea6`): a string-ENUM node param used to render READ-ONLY in the generic inspector (`NPanel` ParamRow is VALUE-type dispatched: string → `<span>`). Fixed by reading the node's declared zod `paramSchema` — a top-level string param that unwraps to a `ZodEnum` now renders an `EnumField` `<select>` (free strings still read-only). Observed LIVE (`inspector-enum-param` e2e, the predicted observation target hit): the axis dropdown drove UI→setParam→DAG→render (mesh span x≈5→y≈5), free-string + material param-rows regression-green. Unlocked `axis` + `RenderOutput.tonemap` / `TransformClip.loop` / `VideoStitch.codec`. Residual (H104-adjacent, deferred): enum params get no keyframe ◇ from the inspector (discrete config, like `muted`). Milestone still OPEN (user chose keep-stacking).

### Boundary B23: Basher (browser, proprietary) ↔ ComfyUI server (GPL-3.0, HTTP API) — the arm's-length AI-render boundary (ComfyUI epic; Inc 1 control passes + Inc 2 connection layer SHIPPED)
**ORIGIN:** 2026-06-24. The ComfyUI keyframe-compiler epic ([[V81]]) makes ComfyUI the render engine on the far side of an HTTP API. Inc 1 made control passes real ([[V82]]); Inc 2 made the connection configurable + observable (settings store + Test Connection). The CORS 403 ([[H120]]) surfaced the moment the browser first tried to reach the server.
**WHY:** Without this boundary tracked, three classes of bug stay invisible. (1) A both-sides failure where Basher's client is correct but the SERVER is misconfigured (no `--enable-cors-header` → 403; wrong URL; server down) gets debugged on the wrong side — `curl` works, the browser doesn't, and you hunt in `HttpComfyUICapability` instead of the launch flags ([[H120]]). (2) A silent stub-fallback: `pickComfyUI` quietly returns the stub when the real server is unreachable, so "it ran" hides "it ran against a 1×1 hash, not a GPU" — the same class as the Inc-1 stub-pixels blocker ([[V82]]). (3) The licensing boundary erodes: if ComfyUI code is ever vendored/forked/bundled into a Basher distributable, the GPL-3.0 copyleft reaches Basher (the arm's-length HTTP posture is what keeps Basher proprietary — design §3).
**HOW:** observe BOTH sides of the boundary. THEIR side: the server's `/system_stats` (reachable + version + device — what `probeComfyUI` surfaces), `/object_info` (node registry; later, the `BasherSchedule` bridge-node presence per Q-E), and the `/history` record of what actually ran. OUR side: the compiled workflow JSON Basher SENT (the snapshot test — the IP, testable without a GPU, design §15). The capability is a session-cached singleton (`getComfyCapability`), reset on a settings change (`resetComfyCapability`) so a new URL re-probes. The Stub capability is the CI gate (deterministic); a real server is a manual/observed validation (this session: `● Connected · ComfyUI 0.26.0 · mps`, and the raw `/prompt`→`/view` green-cube round-trip). **REF:** [[V81]] (the epic invariant), [[V82]] (control passes real on the ONE render core), [[H120]] (the CORS 403 both-sides trap), [[H119]] (the depth-pass traps), `src/core/comfy/index.ts` (`pickComfyUI`/`probeComfyUI`), `src/core/comfy/HttpComfyUICapability.ts` (the HTTP client — `/upload`→`/prompt`→`/history`→`/view`, auth on every call), `src/app/boot.ts` (`getComfyCapability`/`resetComfyCapability`), `src/app/SettingsModal.tsx` (Test Connection — the boundary observation surface), `../projects/comfyui/run-basher.sh` (THEIR launch flags, incl. `--enable-cors-header`), design `docs/COMFYUI-KEYFRAME-COMPILER-DESIGN.md` §3 (licensing) + §10 (Inc 2) + §15 (verify), commits `5019826` (Inc 1) / `0cc2965` (Inc 2). Ground Truth candidate: download ComfyUI source, trace `/prompt` execution + batch handling (for Inc 4's `submitBatch`).
**Silent-failure modes:** server reachable by `curl` but not the browser (CORS/origin 403 — [[H120]]); `pickComfyUI` falls back to the stub on an unreachable server with no surfaced reason (mitigated: `probeComfyUI` reports the error; Test Connection shows it; but the AGENT/render path still silently stubs — surface there too in Inc 3); a saved URL change not re-probing because the session cache wasn't reset (mitigated by `resetComfyCapability` on Save); model licenses (OpenRAIL-M / non-commercial video) treated as covered by the ComfyUI license audit when they are a SEPARATE, stricter gate (design §17); a STRUCTURAL param silently dropped from the compiled path (§7.4 — Inc 4).
**Observation targets:** for any ComfyUI-touching change, ask "did I observe THEIR side (the `/system_stats` / `/history` record) or only infer from my own request?" and "does an unreachable/misconfigured server FAIL LOUDLY (a surfaced reason) rather than silently stub?". Drive the boundary by snapshotting the compiled workflow JSON (deterministic, no GPU) AND, when a real server is up, the `/prompt`→`/view` round-trip (the green-cube method). Inc 1+2 SHIPPED; **Inc 3 SHIPPED** (keyframe-any-param preview + REAL submit `591933e`): a comfy LAYER submits its per-frame compiled workflow when `comfyLiveGenerate` is on; OBSERVED a real SD1.5 frame ("a green cube on a table…", 3998 colours) composited in-app. **The silent-stub (b) is now PARTLY discharged for the layer path** (a submit failure surfaces to assetErrorStore; the opt-in default-off means CI/offline still stub by design, not silently). **NEW [[H121]] caught here:** the HTTP capability had NEVER worked from the browser — `globalThis.fetch` stored on the instance + called as `this.fetchImpl(...)` threw "Illegal invocation" (dormant: tests inject fetch; Test Connection used `probeComfyUI`'s free-variable fetch). Fix = `globalThis.fetch.bind(globalThis)`. Lesson for THIS boundary: the snapshot-JSON gate + Test Connection do NOT exercise the real HTTP submit path — only an actual browser `/prompt` round-trip does; the green-cube method must be run against a live server before claiming the submit works. **NEXT = Slice D — home LOCKED 2026-06-26: the graph-param authoring rows live in a dedicated compositor Controls panel (B24's surface), NOT the NPanel inspector (covered z-45 in VIDEO + local selection). The panel = AE Effect Controls GENERALIZED to "all inputs of the layer's producer pipeline" (source⊕effects); the ComfyUI manifest is the first SOURCE-section renderer. Channel type dispatched EXPLICITLY by valueKind (H104). Design COMPOSITOR §7.1.** → Inc 4 (`submitBatch` BATCHED coherent path + the bridge node + MP4; coherence DELEGATED to the imported workflow — `BasherSchedule` stays the model-agnostic per-frame-array injector, NOT coupled to AnimateDiff).

### Boundary B24: Composition ↔ compositing engine (the Compositor viewer/export composite — spine 1d SHIPPED)
**ORIGIN:** 2026-06-25. Spine 1d landed the live composite viewer (`35433aa`): the comp's ordered layers become pixels on a 2D canvas at the playhead. This is the seam where the DAG (pure metadata: Composition→Layers→Image sources + [[V57]] channels) meets the impure pixel pass (decode + draw). Predicted in design §9 + the [[V83]] status note as the boundary to add WHEN the composite lands.
**WHY:** Without this boundary tracked, four classes of bug stay invisible (design §6/§9). (1) **Layer-order vs z-order drift** — the comp `layers` list is back→front (index 0 = back), the timeline renders REVERSED (front-on-top), and `drawComposite` paints back→top: three orderings that must agree, or the composite stacks wrong silently. (2) **Trim/startFrame off-by-one at frame mapping** — comp frame → source frame goes through `mediaClipFrameAt` (the SAME map the evaluator uses); a parallel/inlined remap drifts preview from source ([[H40]] form). (3) **Blend-mode / opacity parity viewer-vs-export** — the viewer and the export (1e) MUST walk the SAME `planComposite` + `drawComposite`, or what you see ≠ what you render ([[V37]]). (4) **Silent blank** — a decode failure (corrupt media, OPFS read error, unreachable path) that only `console.warn`s leaves that layer invisible with no surfaced reason. **Evaluated-overlay correctness:** the viewer reads keyframed opacity/rotation via `resolveEvaluatedParam` (the renderer-identical path) — NOT raw params — or animation wouldn't show ([[H40]]).
**HOW:** observe BOTH sides. THEIR side (the browser's decode/draw): the actual canvas PIXELS (`getImageData` — the p-compositor-viewer non-uniform check; a screenshot for eyeballing opacity/rotation/blend). OUR side (the plan): `planComposite`'s output (which layers, which source frame, what opacity/transform) — PURE + unit-tested (`composite.test.ts`), the `data-composite-draws` mirror attr on the canvas. The ONE composite core is `src/app/video/composite.ts` (`planComposite` decides visibility/remap; `drawComposite` paints); `CompositeViewer.tsx` wires DAG→inputs (`collectCompositeInputs`, overlays opacity/rotation via `resolveEvaluatedParam`) → decode (`MediaDecodeCapability`, OPFS `storage.read`, cached by `path#frame`) → draw. The export (1e) MUST reuse `planComposite`+`drawComposite` (parity by construction). **REF:** [[V83]] (the epic invariant), [[V37]] (render==viewport), [[V57]] (the evaluated-overlay channels), [[H40]] (read the EVALUATED value, not the raw param), `src/app/video/composite.ts` (the pure core), `src/app/video/CompositeViewer.tsx` (the impure wiring), `src/nodes/MediaClip.ts` (`mediaClipFrameAt` — the ONE time→frame map), `src/core/media/` (the decode capability), `docs/COMPOSITOR-DESIGN.md` §6, commit `35433aa`.
**Silent-failure modes:** layer-order vs z-order drift (3 orderings: `layers` back→front / timeline reversed / draw back→top); trim/startFrame off-by-one if the remap is inlined instead of `mediaClipFrameAt`; ~~blend/opacity parity viewer-vs-export~~ **CLOSED** (`9aac863`, spine 1e) — the impure collect+decode half was lifted into shared `compositeDecode.ts` (`collectCompositeInputs`/`decodeDraws`/`captureCompositeFrame`), so the viewer AND the export walk the SAME collect→planComposite→decode→drawComposite (no second path); PROVEN by observation: an exported MP4's extracted frame is byte-for-byte the viewer's composite (ffprobe: real H.264 1280×720 150f 5.000s); a decode error that only `console.warn`s → that layer silently blank (NOT surfaced to `assetErrorStore` yet — a follow-up); the module-global `bitmapCache` (now shared viewer+export, so the export reuses viewer decodes) never evicts (grows across a long session); ~~no playback/scrub transport~~ **RESOLVED** (`82db85a`) — `VideoTransport` (play/pause/scrub) + the draggable comp ruler now MOVE `timeStore.frame` in-surface, the global playhead range is sized to the comp while in video mode (`compDurationSeconds`, gated on the active `space`, restored on exit → loops at the comp boundary, no 3D leak), and ONE shared global↔comp frame map (`videoTimelineGeometry.globalFrameToCompFrame`/`compFrameToSeconds`/`xToCompFrame`) feeds the ruler playhead + readout + CompositeViewer (H95); NEW silent-failure to watch: a **transient edit** (animated param edited paused with Auto-Key OFF) is held in `transientEditStore` and overlaid by `resolveEvaluatedParam`, but the composite subscribes to the DAG/channels, NOT the transient store → a paused transient preview does NOT redraw the composite (only a real keyed value or a base/channel change does) — diagnostic: "is the value I expect a committed keyframe, or a held transient?"; also a global-duration leak if the sync is ever moved off the `space`-gated effect into an unmount cleanup (the slot stays mounted via display:none); **NEW (inc 2a effects, `4a6702f`):** the compositing engine now applies an EFFECT chain (Image→Image, V58) to each decoded frame — silent-failures: (a) `collectCompositeInputs` reading the layer's DIRECT source instead of walking the effect chain to the base MediaClip (`resolveEffectBase`) → an effect node has no `srcFrames`/`src` → blank layer or a bar shrunk to 1 frame (FIXED `fce9b0c` for the bar; the source resolve walks the chain); (b) effect params read RAW instead of EVALUATED (`resolveEvaluatedParam`, H40) → a keyframed grade wouldn't show (read path is H40-correct, keyframe-ready for 2b); (c) `compositeBitmapKey` missing the `effectsKey` → a graded frame collides with the ungraded cache → stale/wrong pixels (the key folds path#frame#chain); (d) applying effects in the PURE `drawComposite` instead of the impure decode seam → the pure core stops being testable (effects live in `decodeDraws.applyEffects`, drawComposite still draws ready bitmaps); (e) the effect stack built as a one-off instead of on `operatorStack` → add/remove/reorder/mute/serialize duplicated (it reuses the SAME engine via `EFFECT_NODE_TYPES` — V58).
**Observation targets:** for any composite-touching change, ask "did I observe the actual CANVAS PIXELS (getImageData / screenshot) or only the plan?" and "does the viewer and the export walk the SAME `planComposite`+`drawComposite` (parity), or did a second composite path appear?". Drive the boundary by the pure `composite.test.ts` (plan) + the live `p-compositor-viewer` non-uniform pixel check + a screenshot for opacity/rotation/blend. 1d SHIPPED (images, opacity/transform/blend, enabled/solo/trim) + the video-mode transport SHIPPED (`82db85a` — play/pause/scrub + draggable ruler now move `timeStore.frame` in-surface so animation plays; observed via a 2-key opacity scrub: frame 0 brightness 143 → frame 75 @ 0.05 brightness 7 → back 143). 1e export SHIPPED (`9aac863` — Export ▾ MP4/PNG via the shared `compositeDecode.captureCompositeFrame` + the renderAnimation loop/sinks; viewer==export proven by the extracted-frame observation). **SPINE 1a→1e COMPLETE.** Inc 2a first video effect SHIPPED (`4a6702f` — Color Correct, V58 lifted to Image; effect chain applied at the decode seam; observed darken 143→43 / mute 143 / remove). **Inc 2b keyframeable effect params SHIPPED** (`f934231`) — effect-param sub-rows (Brightness/Contrast/Saturation) under a per-effect twirl, each `ParamDiamond` + `useAnimatableField` targeting the EFFECT node (V57/H104); effect channel keyframes drawn on the track via the generalized `TrackKeyframeRow`. The 2a "read path is H40-correct, keyframe-ready for 2b" promise is now DISCHARGED + PROVEN: a 2-key brightness animation (1.0↔0.3) drives the live composite across a scrub (full red → dark), reading the EVALUATED param via the unchanged `collectCompositeInputs` overlay — so the silent-failure (b) "effect params read RAW not EVALUATED" is now exercised by a falsifiable pixel e2e (`p-compositor-effect-keyframe`), not just asserted. **1c.3c-ii Vec2 layer props SHIPPED** (`e18dc8e` infra + `15497ca` UI) — Position/Scale keyframe via a new `KeyframeChannelVec2` + `useAnimatableVec2Field`; `collectCompositeInputs` now overlays `transform.position`/`transform.scale` via `resolveEvaluatedParam` (vec2), so the composite's position/scale are no longer read RAW — an animated 2D transform shows in viewer+export (the prior "position/scale authored until 3c-ii" 1d known-limit is CLOSED; proven by a 2-key Scale scrub squeezing the red region). NEW silent-failure surfaced here: a NEW channel value type (vec2) must be taught to EVERY value-shape gate or a key is silently rejected — the first key (dispatchDirectFirstKey/inferValueType) landed but the SECOND (mutator.timeline.keyframe VALUE_SHAPE_BY_TYPE) was rejected until vec2 was added there too; the tell was "only 1 keyframe after 2 diamond clicks" (caught by the pixel/count e2e, not unit tests). **Inc 3 (compositor) ComfyUIWorkflow layer source — spine SHIPPED** (`40a90d7`) — the EXISTING ComfyUIWorkflow node (already outputs Image) is now a Layer source; `compositeDecode` gains a `kind:'comfy'` branch decoding a DETERMINISTIC stub frame (no server). This adds a 2nd source kind to B24's decode boundary — silent-failures to watch as it grows: (a) ~~the stub cache key `comfy:<nodeId>` does NOT fold params~~ **CLOSED** (`8730c3d`, inc 3 Slice C) — the comfy decode key now folds the graph json + the schedulable params RESOLVED-at-frame (`comfy:<id>#hash({json,vals})`), so an edited workflow OR an animated/bound param re-renders; (b) a real ComfyUI submit is async + fallible (server unreachable → H120 CORS / stub fallback) — the decode seam must surface failure (assetErrorStore), not silently blank — STILL OPEN (the decode is stub-only; the real-submit slice owns this); (c) ~~NODE-MODEL fork~~ **RESOLVED** (`6ec9ca3`, Slice B) — design §6.2 chose EVOLVE: the ComfyUIWorkflow node gains an ADDITIVE `graph` param ({apiJson,meta}, manifest derived on read via `importComfyGraph`); the v0.5 preset path is untouched (additive, no migration). **Inc 3 keyframe-any-param SHIPPED** (`8730c3d`): the COMPILER core (`src/core/comfy/comfyGraph.ts` — `importComfyGraph` manifest + SCHEDULABLE/STRUCTURAL `scheduleHint` §7.4 + `compilePreviewFrame` §7.2, pure + snapshot-tested, the GPU-free IP); new step-interp `KeyframeChannelText`/`KeyframeChannelImage` channels (the vec2-add pattern — teach EVERY VALUE_SHAPE gate or the 2nd key silently drops) bound free-floating to `comfy:<nodeId>.<inputName>`; `compositeDecode` resolves each SCHEDULABLE param at the playhead via `resolveEvaluatedParam` (render-identical, H40) and the stub draws the resolved prompt → composite changes across a scrub (OBSERVED: 'a green cube'→'a red sphere'; e2e `p-compositor-comfy-keyframe`). NEW silent-failures surfaced: the comfy stub BACKGROUND colour derives from the cache key (which folds resolved values) → a continuously-animated NUMBER param flickers the stub colour every frame (a quirky PLACEHOLDER behaviour, replaced by real submit; not a bug); `importComfyGraph`+`resolveEvaluatedParam` re-run per frame (O(params×nodes)) — memoize the manifest if it shows; `inferValueType`/`channelNodeFor` were NOT taught 'text' (intentional — comfy-param authoring will dispatch the channel type EXPLICITLY by the manifest's valueKind, not by guessing a string-vs-color; Slice D). NEXT candidates = inc 3 real `/prompt` submit (surface (b)) + inspector authoring rows for graph params (H104, Slice D) → 1b.2 video decode → agent effect mutator (+ vec2/text in addChannel). **Slice D + generic image-input picker + render-passes-as-project-images + LIVE round-trip OBSERVED (2026-06-26, `247b239`→`f115e26`):** the Controls panel (B24's authoring surface) renders the comfy manifest; an `'image'`-valueKind param is a GENERIC pick-a-project-image / upload affordance — NO ControlNet special path ([[feedback_comfy_image_input_generic]]); render passes save to OPFS `render_<frame>_<pass>.png` as MediaClip image nodes → become pickable (the 3D-scene-as-control-rig on-ramp). **Silent-failure (b) "decode seam must surface failure, not silently blank" is now DISCHARGED, and a deeper trap found = [[H125]]:** the live submit DISPLAY failed (server `execution_success`, in-app stub) from TWO causes — (1) **no in-flight dedup** in `ensureBitmap` → every composite re-render fired another `/prompt` → serial-GPU backlog starved each client submit past its 30s timeout (FIX: `inFlight` map keyed `path#frame`, one decode per key); (2) the submit error went to the view3d-covered `assetErrorStore` banner → invisible in VIDEO ([[H122]] 3rd occ; FIX: app-root toast). **NEW observation targets at this boundary: any decode that calls a SLOW async resource from a render loop MUST dedup in-flight by cache key (a result cache alone starves), AND its failures MUST notify the app-root toast, never a view3d-slot surface (covered in VIDEO/2D).** OBSERVED live: bound depth pass → real img2img frame composited (151 colours, ONE submit, ~6s); the green-cube frame fetched from `/view` proved the round-trip end-to-end. NEXT = Inc 4 (`submitBatch` batched coherent path + `BasherSchedule` + MP4). **Timeline-mirror gap CLOSED [[H129]] (`9e9d934`, 2026-06-30):** a keyed comfy/controller param (V57 channel TARGETING the ComfyUIWorkflow SOURCE node, not the Layer) drew a red diamond in the Controls panel but NO dots in the `LayerTimeline` twirl — the twirl enumerated only a FIXED prop list, never the source's channels. FIX = `collectComfySourceChannelRows` + a read-only `'comfy-prop'` twirl row drawing dots with `nodeId=sourceNode`. **NEW observation targets at this boundary: (1) a dopesheet must enumerate channels by the SUBJECT a row represents (the layer's SOURCE), never assume `channel.target === rowOwnerId`; (2) a video-layer's keyframe surface is the compositor `LayerTimeline`, NOT the 3D `TimelineDrawer` — in VIDEO space `video-slot` z-45 COVERS the 3D drawer z-20 (H122 family, 2nd occurrence: toast slot, then timeline-drawer → a click on the covered surface is intercepted / "breaks the page").**
