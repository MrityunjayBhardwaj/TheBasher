...
## V47 — Scene environment (HDRI/IBL) is a DAG-resident Scene-level datum bound to `scene.environment`; it is NOT editor chrome, so it survives the render hide-pass and flows into the offscreen render automatically [NOT YET IMPLEMENTED — UX-BACKLOG #9]
...
**Enforcement (planned):** node unit (`Scene.evaluate` folds env params → `SceneValue.environment`; default `none`); renderer/e2e (env set → `scene.environment` non-null; background toggle sets/clears `scene.background`; the env flows into `renderToImage` output; an old project with no env param renders identically = back-compat). REF (grounded 2026-06-15 by the light-pipeline trace): `src/nodes/Scene.ts` (params + evaluate), `src/nodes/types.ts` (`SceneValue.environment`), `src/viewport/SceneFromDAG.tsx:378-391` (light dispatch — the sibling site; env mounts where the Scene value is consumed), `src/render/renderToImage.ts:139-151` (hide-pass excludes `scene.environment`), `src/app/asset/bakedTextureLoader.ts` (OPFS→texture Suspense pattern to mirror for `file` source → blob URL → drei `files`), drei `<Environment>` (loader + PMREM + background). Decision source: AskUserQuestion 2026-06-15 (source = both presets + import; shape = Scene-level param). Status: **NOT YET IMPLEMENTED.** Branch `ux-overhall`.
