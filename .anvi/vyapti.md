...
**Status:** IMPLEMENTED for drill 2026-06-14 (`c5f18fc`; merged into `ux-overhall` + pushed). `keyByGltfNodeIndex` persisted on `GltfAsset` (additive `.default({})`, no schema bump); `GltfAssetR` stamps the clone; `buildGltfDrillChain` reads stamps (name-match fallback for pre-2026-06-14 saves). NOT YET applied to the override paths (the wider pre-existing bug noted in [[H90]]) — that is the next occurrence; the stamp is already present on every clone object, so the fix is a consumer change only.
