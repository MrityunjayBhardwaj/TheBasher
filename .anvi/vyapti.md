...
**Status:** ENFORCED for v0.6 #4 W3 (calm LIGHT). The contrast matrix (`contrastMatrix.test.ts`) imports the real Tailwind config + a `border-token-gate` (3:1) — both re-gate automatically on a token change; the `#57` over-canvas `it()` + `p57` e2e were re-grounded white→black. GROUNDED by observation (the matrix failed during W3 iteration until the tokens cleared; falsified post-commit — restoring neon accent fails 13 rows + the focus-ring gate).

**REF:** `tailwind.config.ts` (the darkened semantic tokens), `src/a11y/contrastMatrix.test.ts` (TOKEN mirror + F2 drift gate + border-token-gate 3:1 + the re-grounded #57 dark-backdrop `it()`), `tests/e2e/p57-bright-scene-contrast.spec.ts` (re-grounded worst-case + glyph-pick + falsification), `src/viewport/viewportColors.ts` (calm canvas tint, named); CONTEXT D-07; W3 commit `29a3afe`. Sibling [[V34]]/[[V35]] (the W3/W4 chrome-discipline cluster); [[H27]] (the re-validation trap the corollary instantiates).
