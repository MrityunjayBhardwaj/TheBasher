...
**The invariant:** once the chrome panels stop being grid columns/rows and become absolute islands over a full-bleed viewport (UX-BACKLOG #2), the layout no longer reserves space for anyone — a panel and a viewport widget can occupy the same pixels with no error, just visual collision. So: (1) all island geometry (gaps, panel widths, the reserved bottom-widget band, the centered-surface side reserve) lives in ONE module `src/app/layoutIslands.ts` — never hardcoded per surface (the V34/V45 "one home" face). (2) Side islands are TOP-anchored and stop short of the bottom (`BOTTOM_BAND`) so the already-floating bottom-right orbit gizmo + Persp/Ortho and the bottom-center stack stay clear — the lowest-coupling fix (no widget dodges a panel). (3) Any VIEWPORT-CENTERED surface (the toolbar pill, the agent/timeline stack) caps its width by `CENTER_SIDE_RESERVED` so it can't slide under a side island at a narrow width. (4) An island that overlaps `<main>` stops `onContextMenu` propagation so a panel right-click doesn't pop the viewport Add menu.

**Breaks when:** a new floating panel hardcodes its own width/offset instead of consuming `layoutIslands.ts` → it drifts from the others when one changes; a centered surface omits the `CENTER_SIDE_RESERVED` cap → it slides under a side island at narrow widths (observed: the toolbar's right end disappeared under the inspector at 1100px before the cap); a side island goes full-height → it covers the bottom-right gizmo/Persp-Ortho (they live within ~120px of the bottom edge); a panel mounted inside `<main>` forgets `stopPropagation` on contextmenu → right-clicking it pops the Add menu. This is the inverse of the [[H91]]/[[V45]] family (a relocated surface silently overlaps another) generalized from spawned menus to the whole chrome layout. Type: STRUCTURAL (chrome-geometry). Sibling of [[V35]] (reveal reachable), [[V39]] (one glow reused), [[V45]] (one placement primitive).

**Enforcement:** `tests/e2e/ux2-floating-islands.spec.ts` — viewport full-bleed (no reserved side columns / bottom rows); each side island is `position:absolute`, edge-hugging, top-anchored, bottom-clear; right-click on a panel does NOT open the Add menu (control: empty viewport does); the agent+timeline are a centered stack (chat above timeline); present mode hides every island. Cross-ref [[V45]]/[[H91]] (the overlap family this generalizes), dharana [[B11]] (chrome re-validation). REF: `src/app/layoutIslands.ts`, `src/app/Layout.tsx` (island mounts), `src/app/FloatingViewportToolbar.tsx` (`CENTER_SIDE_RESERVED` cap), `src/app/LeftSidebar.tsx` + `src/app/NPanel.tsx` + `src/app/AgentDock.tsx` (transparent inner panels), `src/viewport/Viewport.tsx` (orbit-hint reposition). Commits: side islands + bottom islands (UX #2 slices 1+2). Branch `ux-overhall`.
