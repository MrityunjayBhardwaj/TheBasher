...
**Status:** IMPLEMENTED for PRIMITIVES (v0.6 #2, issue #178, 2026-06-07 — W0-W5 on branch `feat/v06.2-material-editing`): the IR is `InlineMaterialSpec` (widened, `src/nodes/types.ts`); the ONE adapter is `openpbrToThree` (`src/app/material/openpbrToThree.ts`, grep-gated); the one leaf renderer is `usePrimitiveMaterial` (`SceneFromDAG.tsx`); migration v2→v3 + the V10/H14 three-layer guard live in `src/nodes/materialSchema.ts`; texture maps round-trip through OPFS (`attachMapFromFile`). PARTIAL: per-submesh glTF (W6 — the glTF child does NOT own its material; per-`(child,slotIndex)` override) NOT YET BUILT. v0.7 still adds the TSL backend + React-Flow editor "build-B". Cross-refs [[V29]] (one EvaluatedMesh face / one read producer), [[V9]] (materials are data not code), [[V10]]/[[H14]] (the widen is a migration → IDENTITY-default guard at every consumer; the paramPath rename's full span = [[H74]]), [[V20]]/[[H36]] (one material writer), dharana [[B19]], DCC-reference §11a. REF: memory `project_material_architecture.md` + `session_2026-06-07_v062-w0-w5-shipped.md`; `.planning/phases/v06.2-material-texture-editing/{CONTEXT,RESEARCH,PLAN}.md`; commits `6b43dda`/`6551617`/`ff5776e`/`4d91274`/`674d3d0`/`877185a`; THESIS §58/§59/§741/§747.
