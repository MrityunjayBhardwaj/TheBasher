...
### V28 [NOT YET IMPLEMENTED]: An override node's per-field "authored" set is EXPLICIT (carried, not derived from value≠default), sparse, and SHARED across override domains — because Basher's params are single-tier (seeded with the source value), the R-4 trap
...
**Status:** PARTIAL — implemented for GltfChild TRS only (`resolveGltfChildTransform.ts` manual band). #124 generalizes: extract `src/core/override/overrideSet.ts`, retrofit GltfChild (regression-guarded), apply to MaterialOverride (`resolveMaterialOverrideFields` swaps map-presence → explicit-set ∪ map-aware-fallback). Backward-compat: empty set = legacy #99 map-aware behavior (D-03). The coarse "flatten / ignore source material" toggle is a SEPARATE primitive (D-05; Blender View-Layer Material Override / Houdini wholesale-assign), not part of the per-field set.

**Enforcement (target):** `overrideSet.test.ts` (sparse + immutable + merge-picks-override-only-where-set); GltfChild p7.7/p7.11/p7.12 stay green through the retrofit (the "2nd consumer justifies the module" proof); `materialOverrideMerge.test.ts` adds forced-metalness-over-a-map. Cross-ref [[H59]] (map-aware tint — the default when a field is NOT in the set), [[V20]]/[[H36]] (single-writer material clone), the GltfChild R-4 value-equality trap, [[V27]] (sibling: content-addressed group; both are "the model carries the structural fact explicitly, don't infer it"). REF: `.planning/phases/124-material-override-primitive/CONTEXT.md` (decisions D-01..D-06, Houdini+Blender grounding) + PLAN.md. Issue #124 (successor to #99 D-06).
