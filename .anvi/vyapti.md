...
**Enforcement (SHIPPED S1–S3, #178, 2026-06-16, `ux-overhall`):** `gltfMaterialToOpenpbr.ts` (clone→IR oracle, S1); `gltfJsonMaterialToOpenpbr.ts` + `GltfChild.materials` + `buildGltfImportOps` capture (S2); `overlayDagMaterial` in `SceneFromDAG.tsx` (S3). Observed: cicada renders pixel-identically; editing `materials[0].base.color` repaints the clone (`tests/e2e/ux-gltf-material-dag.spec.ts`). Status: **PARTIAL** — S1–S3 render/parity done; S4 (inspector editing), S5 (texture-map capture, currently maps=null), S6 (animation + serialization) PENDING.
