...
**Status:** NOT YET IMPLEMENTED (target: phase `v06.2-material-texture-editing`, W1 builds the IR + the `openpbrToThree` adapter; v0.7 adds the TSL backend + React-Flow node editor "build-B"). Cross-refs [[V29]] (one EvaluatedMesh face / one read producer), [[V9]] (materials are data not code), [[V10]]/[[H14]] (the widen is a migration → IDENTITY-default guard at every consumer), [[V20]]/[[H36]] (one material writer), dharana [[B19]], DCC-reference §11a. REF: memory `project_material_architecture.md`; `.planning/phases/v06.2-material-texture-editing/{CONTEXT,RESEARCH,PLAN}.md`; THESIS §58/§59/§741/§747.
