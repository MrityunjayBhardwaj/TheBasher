...
> **Re-censused 2026-07-28, after C5 (#388) — §1.1's table above is HISTORY, and all three of #356's witnesses are discharged on the declaration side.** Measured at branch head, not remembered: `GltfAsset` now declares `['mesh','driver','material']`, `AmbientLight` `['driver']`, `ScatterNode` `['mesh','driver','material']`. **None of the three still advertises `'transform'` or `'constraint'`**, so the "+ Follow Path that moves nothing" symptom #356 was opened on is gone for all three. **#356 is very likely fixed-but-not-closed — verify before working it.** (`ScatterNode`'s data half also landed separately, `cde2242`/#362: `ScatterValue` carries no TRS.) The declaring set is now **15**, not 17, and ten of those fifteen are retired relics that throw on evaluate — kept only so the load ladder can normalize a saved node before the format migration splits it.
>
> **What is still genuinely fused is exactly two nodes:**
>
> | node               | how it is fused                                                                                                                                                                                    | issue                  |
> | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
> | **`GltfChild`**    | `GltfChildValue` carries `position/rotation/scale` (`types.ts:650-652`) **and** it declares `['transform','constraint','driver','material']` — fused at both the value and the inspector, and LIVE | #389 (blocked on #367) |
> | **`ModifiedMesh`** | declares no transform section, but `ModifiedMeshValue` carries TRS (`types.ts:530-532`) — fused at the **value** level only                                                                        | #415                   |
>
> **A correction worth carrying forward:** several notes list `GltfAsset` as still wrongly carrying TRS. It does not, and never did at the value — `GltfAssetValue` is `assetRef` plus an animation map, with no TRS. Its pose correctly lives on the import-root `Group`, exactly as §1.1 concludes below. `AmbientLight` is likewise not a split candidate at all: it has no pose to separate, which is why C3 (#386) retired it only partially.
...
**Perf is not a counterargument.** Decomposing the MODEL does not force CPU skinning: the renderer can recognise `Object + meshData(weights) + skeleton` and build a three.js `SkinnedMesh` with GPU skinning — the same shape as the **value half** of handle-not-buffers **[S2]**, which the DAG does uphold: what flows is a `GeometryRef`, never inline arrays. ⚠️ **Re-graded 2026-08-09 — S2 is PARTIAL, not COVERED** (this line said COVERED until #606). The value half is real; the **operator half is not** — `geometryRegistry.build()`'s `array`/`mirror` arms resolve the source to real buffers, `clone()` per copy and `mergeGeometries`, so copying does not copy a reference. The argument above only needs the value half and survives the correction; the citation does not. → **#606**. Both DCCs decompose the model and deform efficiently. **The model never dictated the execution.**
...
- **The pin that ends the class:** _every node declaring `'constraint'` evaluates to a value carrying `position`_ — a red test. This is what the current `inspectorSectionsRegistry.test.ts` should have been (it only pins that mesh-primary nodes _lead_ with `'mesh'`, which is why glTF sailed through).
  ...
  > **2026-07-28 — this pin is now the highest-value unbuilt thing in the epic, and the reason is a count.** The section-level half worked: all three of §1.1's phantom-`transform` witnesses were fixed, and `inspectorSectionsRegistry.test.ts:242` now pins the converse (posable ⇒ offers constraints). But the **general** shape — _a param or section a surface lets you edit must have a consumer that reads it_ — is unpinned, and six open issues are now the same bug in different clothing: #489 (a baked mesh's `scale` row is editable, the renderer draws identity), #478 (ortho `zoom` authored and saved, no renderer reads it), #356 / #372 (glTF constrainable in the UI, nothing to move), #474 (a held edit on a curve's data params can't repaint it), #484 (a held lens edit never reaches the shot).
  >
  > Six instances at one boundary — _what a node declares editable_ vs _what the render/read road consumes_ — is past the threshold where these stop being independent bugs and start indicting the boundary. Each was found by a user or a review, one at a time, and each fix is local. The generalized pin (walk every registered node's declared params + sections, assert each has a reader) converts the whole class from serial discovery into one red test, and is cheap next to six more rounds of finding them by hand. **Worth its own issue before the count reaches ten.**
