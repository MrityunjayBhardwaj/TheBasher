...
| `src/viewport/DiffOverlay.tsx:120` (the diff ghost)   | evaluated **value** | `if (followed && 'position' in rec)`   |
...
| **`GltfAsset`**          | ❌ **none**                                | ❌ `{ kind, assetRef }` (`types.ts:575-576`) | ❌ **inert**                 |
...
**Two mature DCCs, two entirely different mechanisms, the same joint.** That convergence is the evidence: the boundary is the domain's, not Blender's taste. Basher is the outlier, and the outlier is the one generating the defect family in §1.2.

**Survey bar (honest):** Maya (transform↔shape), C4D and 3ds Max are **not surveyed** for this split. Below `dcc-reference.md`'s own five-DCC bar. Recorded at the lower bar because the structural finding is confirmed by two independent references; Maya's transform↔shape is the highest-value confirmation and should be added in Phase 0.
...
**Provenance:** the glTF SOP node page is an **[EXTERNAL]** WebFetch (`sidefx.com/docs/houdini/nodes/sop/gltf.html`), not project-scoped. The packed-primitive semantics it rests on are **[in-envelope]** (`ref/houdini/SOP.md` §6). **Phase-0 gate: a proper Ground Truth pass on the glTF SOP before this section is treated as locked grounding.**
...
**Phase 0 — Surface map, locked decisions, no code.**
...
**Phase 4 — size-vs-scale (#231 D).** `size` is data; `scale` is Object. Un-conflates `getManipulable`'s `sizeFallback` (`Gizmo.tsx:168`).
...
2. **Three things currently play Object** — `Group`, `Null`, `Transform`. Does `Empty = Object with no data` collapse all three? Does `Transform` survive at all?
3. **`GltfChild`** — data, or an Object in its own right? #231 E's actual question; a glTF's children are _objects_ in Blender's import.
4. **Channel targeting (V57)** — transform channels target the Object, data channels the data node. Confirm the enumerator scopes correctly and that migration re-targets each to the right half. **Getting this wrong orphans every animation in every saved project.**
5. **`ScatterNode` / `BakedMesh` / `Character`** — which side, and do they split at all? **`BakedMesh` is now better understood (§2.1.1): it is Basher's _Unpack_** — the explicit materialize-to-real-buffers step, Houdini's inverse-of-pack. That reframes the question from "which side is it on" to "**is `BakedMesh` an Object at all, or purely a data node?**" Its value carries identity TRS _and_ geometry _and_ material — a fused shape by construction. Under the split it looks like **data** (a `GeometryRef` + a material spec), with the identity TRS being exactly the fusion artefact the split removes.
6. **Maya's transform↔shape** — add as the third reference before locking §2.
7. **Does fan-out survive the evaluator's assumptions?** Two Objects consuming one data node — verify against V42/V43 (structural sharing) and the params-hash WeakMap memo (`evaluator.ts:52-64`).
