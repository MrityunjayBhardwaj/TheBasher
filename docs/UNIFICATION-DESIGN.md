...
**Phase 2 — Native mesh onto the road.** Enumerate channels by `target = mesh dagId`; renderer + `resolveEvaluatedMesh`/`resolveEvaluatedTransform` read the resolver directly (not via the wrapper). Author via direct first-key (mirror `dispatchCameraFirstKey`), no `addLayer`. *Observe:* keyframe a Box's position + roughness with autoKey; scrub; viewport == still == animation render.
...
**Phase 5 — Retire AnimationLayer + migrate.** Remove the wrapper from producers (`dispatchMutator`/`resolveEditTarget`/`autoKeyCommit`), add the load-time migration + shim, delete the node (or keep a deprecated read-shim one release). *Observe:* open an old `.basher` with a layer; it animates identically; new edits create no layer.
