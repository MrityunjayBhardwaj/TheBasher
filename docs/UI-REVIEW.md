...
| **CI-1 / FLAG-2** count-constant ≠ pixels-restored | **RESOLVED — PASS-manual (user A2 scrub 2026-05-17)** | The strip-restore observation W9 could not automate was performed: 6ch×24kf seed, playhead swept repeatedly — diamonds intact under/behind the line, no erase trail. No code change needed for strip-restore. The 1 skipped Playwright spec remains skipped — a *permanent* jsdom limitation (cannot run rAF+canvas), not a pending deferral; the observation was satisfied manually. **Residual:** the underlying *count ≠ pixels* class (mirror-attr asserts cardinality, not on-canvas visibility) is NOT closed by this scrub — it produced a separate concrete escape, **F-7** (terminal-keyframe clip: count=2 / 1 visible), fixed `4762d49` and catalogued as **hetvabhasa H35**. |
