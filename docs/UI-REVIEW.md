...
- **F-5 — ComfyStatusIndicator state change is not announced.**
  ...
  - text but no `aria-live`. A non-visual user gets no notification when the AI
    bridge goes live/offline. Spec §8.4.4 exempts the _idle_ visual contrast but
...
- **Pillars 1–4 (Layout, Typography, Color, Spacing): `PIXEL-OBSERVED PASS`
  ...
  are _code-derived_, not pixel observation, and a 2D-canvas raster cannot be
...
Re-derived from the _assembled UI audited this session_ (source read of all
...
| **c-1** zoom-% readout never updates                | CODE-FIX (built W10) _(SPEC-AMEND rejected → forced CODE-FIX)_                            | **`afd88b6` — BUILT.** Mini-checkpoint dispositioned by user → BUILD IN W10. Real signal pipeline: `viewportStore.cameraZoom` + pure unit-tested `cameraDistanceToZoomPercent` + `OrbitControls.onChange` writer in `Viewport.tsx` (V8-clean: UI-projection-store write, file-rooted ban covers only DAG dispatch primitives — same in-viewport write class as the long-standing `useSelectionStore.getState().clear()` precedent) + R3 TopToolbar live readout (stays disabled — §5.3 specifies a zoom % _display_, not a zoom-input dropdown; §5.3 NOT amended). Observed: vitest 21/21 (+7), e2e p6-w10-ui-review 2/2 (readout DOM text observed 100%→200%→50%).                                                                                                                                                                                                                                                |
| **c-2** close == delete                             | DEFER → v0.6 _(SPEC-AMEND rejected → forced CODE-FIX; CODE-FIX = new-capability → DEFER)_ | **DEFER → v0.6.** Mini-checkpoint dispositioned by user → DEFER to v0.6. Rationale: a non-destructive close needs a new open-tabs-vs-storage session abstraction — a project-lifecycle redesign, out of audit-wave scope. Spec §5.1 anatomy stays the forward contract (NOT a SPEC-AMEND, NOT a silent divergence); the v0.5 destructive-close gap is roadmapped to v0.6, tracked in the v0.6 roadmap note below. The `window.confirm` guard at `ProjectTabs.tsx:104-106` remains the v0.5 mitigation until then.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **CI-1 / FLAG-2** count-constant ≠ pixels-restored  | **RESOLVED — PASS-manual (user A2 scrub 2026-05-17)**                                     | The strip-restore observation W9 could not automate was performed: 6ch×24kf seed, playhead swept repeatedly — diamonds intact under/behind the line, no erase trail. No code change needed for strip-restore. The 1 skipped Playwright spec remains skipped — a _permanent_ jsdom limitation (cannot run rAF+canvas), not a pending deferral; the observation was satisfied manually. **Residual:** the underlying _count ≠ pixels_ class (mirror-attr asserts cardinality, not on-canvas visibility) is NOT closed by this scrub — it produced a separate concrete escape, **F-7** (terminal-keyframe clip: count=2 / 1 visible), fixed `4762d49` and catalogued as **hetvabhasa H35**.                                                                                                                                                                                                                           |
...
hold** — no fix deleted, merged, or created a chrome _surface_; every fix
...
_unifies_ the Viewport + Layout selection-summary source, **strengthening**
...
`OrbitControls.onChange` writer + a live binding on the _existing_ R3
...
_Generated by `/anvi:ui-review` (W10). §7 = D-W10-1 inline fix run resolution
ledger. All findings terminal: c-1 BUILT (`afd88b6`, user disposition),
c-2 DEFER → v0.6 (roadmapped §7). Zero findings OPEN; zero §1 spec edits._
