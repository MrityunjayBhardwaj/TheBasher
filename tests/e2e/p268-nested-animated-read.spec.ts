// p268 — a nested node's animation is READ where it is DRAWN.
//
// Every node of an import is nested (under the import Group). The renderer overlays a nested node's
// channels, and those of its nested ancestors (#266); the read the gizmo and inspector stand on did
// so only for top-level scene children, so selecting an animated imported node put the gizmo on the
// static pose while the mesh moved.
//
...
//
// REF: src/app/resolveWorldTransform.ts (walk overlays every depth), src/app/resolveEvaluatedTransform.ts
//      (finds a nested node); issues #268, #266, #1051.
...
const keyed = (target: string, from: number[], to: number[]) => ({
  type: 'addNode',
...
async function importNestedCube(page: Page) {
  ...
  await page.evaluate(async () => {
    ...
    await w.__basher_ingestGltfFolder!([{ relativePath: 'nested-cube.gltf', bytes }], 'p268');
  });
...
test('#268 — the gizmo follows a nested object keyed on its own position', async ({ page }) => {
  test.slow();
  const shape = await importNestedCube(page);
  await dispatch(page, [keyed(shape.objectId!, [1, 0, 0], [5, 0, 0])]);
  await page.evaluate(
...
test('#268 — the gizmo follows a nested object whose nested parent is keyed', async ({ page }) => {
  test.slow();
  const shape = await importNestedCube(page);
  await dispatch(page, [keyed(shape.parentId!, [0, 3, 0], [0, 5, 0])]);
  await page.evaluate(
    (id) => (window as unknown as W).__basher_selection!.getState().select(id),
    shape.objectId!,
  );
