...
test('median / active / individual pivot modes drive the multi-gizmo', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    d.dispatch(
      { type: 'setParam', nodeId: 'n_box', paramPath: 'position', value: [0, 0, 0] },
      'user',
      'p',
    );
  ...
  const median = await page.evaluate(
    () => (window as unknown as W).__basher_gizmo_multi?.() ?? null,
  );
  ...
  const positions = await page.evaluate(() => {
    ...
    return {
      box: n['n_box']?.params?.position as number[],
      box2: n['n_box_b']?.params?.position as number[],
    };
  });
